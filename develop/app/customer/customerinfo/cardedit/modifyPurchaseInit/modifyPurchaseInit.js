...
$(document).ready(function () {
    params = getParams();
    $('#buyNum').val(params.purchaseNum);
    $('#packNum').text(params.packNumber);
    $('#totalNum').text(params.totalNum);
});
...
$('#buyNum').bind('input propertychange change', function () {
    ...
    $('#buyNum').val(buyNum);
    var packNum = $('#packNum').text().replace(/[^0-9]/g, '');
    console.log(buyNum)
    if (buyNum.length > 3) {
        ...
        layer.msg("请输入大于0的购买数量!");
    }else {
        var totalNum = Number(buyNum) * Number(packNum);
        if (totalNum < params.consumeNum){
            layer.msg("总次数需要大于已消费次数!");
        }else{
