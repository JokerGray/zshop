(function($) {
    ...
    // var jumpParams="eyJqdW1wQ29kZSI6Im5ld3MwMDMiLCJhcnRpY2xlSWQiOjE3MjIsInR5cGVDb2RlIjoxMDAzfQ==";
    ...
    function articleInit(macId) {
        ...
        reqAjaxAsync('cms_new/queryArticleDetail', param).done(function(resData) {
            if (resData.code == 1) {
                ...
                $(".video-title").html(obj.articleTitle);
                ...
                if (obj.previewImgType == 5 && obj.scCmsResourcesList.length > 0) {
                    var oVideo = obj.scCmsResourcesList[0];
                    // var sHtml = '<video width="100%" src="' + oVideo.videoUrl + '" poster="' + oVideo.resourcesUrl + '" controls="controls"></video>'
                    var sHtml='<div class="zy_media"><video controls="controls" x5-playsinline="" playsinline="" webkit-playsinline="" poster="'+ oVideo.resourcesUrl +'" ><source src="'+oVideo.videoUrl+'" type="video/mp4"> 您的浏览器不支持HTML5视频</video></div>'
                    $(".video-box").html(sHtml);
                }
                $(".article-box .like .count").html(obj.praiseNumber);
                //踩数量
                $(".article-box .unlike .count").html(obj.retortNumber || 0);
                $(".video-play-count").html(obj.visitorNum + "次播放");
                //推荐列表
                ...
                if (!isNull(obj.scCmsResourcesList)) {
                    $.each(obj.scCmsResourcesList, function(i, v) {
                        if (v.resourcesType == "video" || v.resourcesType == "cover") {
        ...
        if (recList.length > 0) {
            ...
            for (var i = 0; i < len; i++) {
                ...
                sHtml += '<div class="info">' +
                    ...
                    '</div></div>'
                sHtml += '<div class="poster"><img src="' + recList[i].scCmsResourcesList[0].resourcesUrl + '">' +
                    '</div></li>';
            }
