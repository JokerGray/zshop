      ...
      if (!shop) {
        if (shopId == undefined) {
          ...
          util.getShop(loginRes.id, shopId).then(function (res) {
            ...
            _this.getPresaleByCondition(shopId)
          })
        }
      } else {
        if (shopId == undefined || shopId == '' || shopId == null) {
          ...
          _this.getPresaleByCondition(shop.id)
        } else {
          if (shopId == shop.id) {
            _this.setData({
              merchantId: shop.merchantId || "",
...
/*********************************socket消息 */
  ...
  getPresaleByCondition(shopId){
    console.log('进入getPresaleByCondition---------------------shopId', shopId)
    console.log('进入getPresaleByCondition---------------------hostId', wx.getStorageSync("hostId"))
    var _this = this
    app.util.reqAsync('shopOrder/getPresaleByCondition', {
      ...
      if (res.data.data != null && res.data.data.orderStatus == 2) {
        ...
        _this.setData({
          ...
          dingDanTotalNum: res.data.data.scPresaleInfoList.length
        })
      } else {
        _this.setData({
          showMaiDan: false
  ...
  addGoodToCartFn: function (e, flag) {
    ...
    if (purchaseType == 6) {
      ...
      util.reqAsync('shopOrder/getPresaleByCondition', {
        ...
        if (res.data.data) {
          for (var i = 0; i < res.data.data.scPresaleInfoList.length; i++) {
        ...
        if (chartNum) {
          ...
          console.log("购物车有此留店商品 +chartNum:" + chartNum + "现在加购" + updateNum + "总" + totalcart)
        } else {
          totalcart = updateNum
          console.log("无此留店商品 +chartNum:" + chartNum + "现在加购" + updateNum + "总" + totalcart)          
        }
...
// 是否弹出 选规格弹框 （默认规格不弹）
    ...
    util.reqAsync('shop/shopCartList', {
      ...
      if (res.data.data.length != 0) {
        ...
        for (let stockIndex in stockMap) {
          ...
          // console.log("goodMap[goodsId].number" + goodMap[goodsId].number + "====>stockMap[stockIndex].number" + stockMap[stockIndex].number);
        }
        this.setData({
          goodMap: goodMap,
          stockMap: stockMap,
  ...
  //旧版
  shopCartList2: function (customerid) {
    // this.setData({
    //   shoppingCart: {},
    //   chooseGoodArr: []
    // })
...
/**socket消息 确认下单 */
      // wx.sendSocketMessage({
      //   data: that.data.presaleId + ',confirm order'
      // }) 
/**socket消息 确认下单 */
  ...
  changeOrdr: function (e) {
    ...
    for (var a in this.data.bugGoods) { //已下单商品
      if (this.data.bugGoods[a].stockId) {
        arrList.push({
          ...
          purchaseType: this.data.bugGoods[a].purchaseType,
          accountRecordId: this.data.bugGoods[a].accountRecordId
        })
      } else {
        arrList.push({
          goodsServiceId: this.data.bugGoods[a].goodsServiceId,
    ...
    for (var b in this.data.cartGoodsList) {
      if (this.data.cartGoodsList[b].stockId && this.data.cartGoodsList[b].stockId != undefined && this.data.cartGoodsList[b].stockId != 'undefined' && this.data.cartGoodsList[b].stockId != null && this.data.cartGoodsList[b].stockId != "null") {
        ...
        arrList.push({
          ...
          accountRecordId: this.data.cartGoodsList[b].accountRecordId
        })
      } else {
    ...
    app.util.reqAsync('shopOrder/uptPresale', {
      ...
      if (data.data.code == 9) {
        ...
        // wx.navigateTo({
        //   url: '../../../pages/myHome/shopOrder/orderDetail/orderDetail?activeIndex=0&shopId=' + this.data.shopId + '&userId=' + this.data.userId + '&presaleId=' + data.data.data + '&facilityId=' + this.data.facilityId + '&merchantId=' +this.data.merchantId
        // })
        this.clearShopCartFn();
        // 生成订单 socket消息跳转详情
        wx.sendSocketMessage({
          data: data.data.data + ',confirm order'
        })
      }
    }).catch((err) => {
      wx.showToast({
  ...
  oneBuy: function (e) {
    ...
    if (cartGoodsList.length > 0) {
      ...
      console.log('第一次下单---------------桌号',this.data.facilityId)
      app.util.reqAsync('shop/submitShopOrderV2', {
        ...
        if (data.data.code == 9) {
          ...
          console.log("第一次提交订单成功orderId==================>", data.data.data.orderId)
          _this.setData({
            presaleId: data.data.data.orderId
          })
          ...
          // wx.redirectTo({
          //   url: '../../../pages/myHome/shopOrder/orderDetail/orderDetail?activeIndex=0&shopId=' + this.data.shopId + '&userId=' + wx.getStorageSync("hostId") + '&presaleId=' + data.data.data.orderId + '&facilityId=' + this.data.facilityId + '&merchantId=' + this.data.merchantId
          // })
        }
      })
    } else {
          ...
          if (data.data.data.orderType == 1) { //订单(小程序为1)
            if (data.data.data.orderStatus == 3) { //已结算
              this.setData({
                ...
                paystatus: data.data.data.payStatus
              })
  ...
  goOfflineOrderDetail(){
    let _this = this
    wx.navigateTo({
      url: '../../../pages/myHome/shopOrder/orderDetail/orderDetail?activeIndex=0&shopId=' + _this.data.shopId + '&userId=' + _this.data.userId + '&presaleId=' + _this.data.presaleId + '&facilityId=' + _this.data.facilityId + '&merchantId=' + _this.data.merchantId
    })
  },
  // 下拉重新获取留店商品
  onPullDownRefresh() {
