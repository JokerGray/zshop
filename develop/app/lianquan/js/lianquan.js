...
// 获取店铺ID,选择店铺后刷新
var paramUserId=getUrlParams('userId');
var SHOPID='',
    anchorType='';
...
Showcontent.prototype={
    ...
    getParams: function() {
        ...
        $.ajax({
            ...
            dataType: "json",
            data: {
                "cmd": 'game/findShopList',
                "data": JSON.stringify(queryShop),
            ...
            success: function(re) {
                ...
                for(var i=0;i<re.data.list.length;i++){
                    if(i==0){
                        shopHtm+='<option selected="selected" value="'+re.data.list[i].shopId+'">'+re.data.list[i].orgName+'</option>';
                    }else{
                        shopHtm+='<option value="'+re.data.list[i].shopId+'">'+re.data.list[i].orgName+'</option>';
                    }
                }
                $('#myShop').html(shopHtm);
                ...
                $('#myShop').on('change',function(){
                    clearPage();
                    console.log("序号"+$('.clint-type.active').index())
                    setTimeout(function(){
                        $('.clint-type').eq(0).trigger('click');
                    },500);
                })
                // 选择主播主脸
                $('.clint-type').unbind('click');
                $('.clint-type').on('click',function(){
                    clearPage();
                    $(this).addClass('active').siblings().removeClass('active');
                    anchorType=$(this).attr('data-type');
                    SHOPID=$('#myShop').val();
                    console.log(SHOPID);
    ...
    //							查看主脸或主播，判断是否有默认信息
                        ...
                        case 4:
                            ...
                            var uploadbtn3 = $("#idcardbtn3"),
                                ...
                                uploadbtn5 = $("#idcardbtn5");
                            runupload(uploadbtn3);
                            runupload(uploadbtn4);
                            runupload(uploadbtn5);
        ...
        if (contentFlag) {
            ...
            $.ajax({
                ...
                success: function(re) {
                    ...
                    setTimeout(function() {
                        $('.clint-type.active').trigger('click');
                    }, 2000);
                },
                error: function(re) {
                    console.log(re.msg);
    ...
    sendApply: function(ifreapply) {
        ...
        if (!ifreapply) {
            ...
            var imgurl1 = imghidden1.attr("imgsrcs") || $($(".idcardback")[0]).children("img").attr("src"),
                ...
                imgurl3 = imghidden3.attr("imgsrcs") || $($(".idcardback")[2]).children("img").attr("src");
        }
        console.log(imgurl1 == "");
        datas.frontUrl = imgurl1;
        datas.contraryUrl = imgurl2;
        datas.holdPhotoUrl=imgurl3;
        ...
        if(!idReg.test(datas.identificationNo)){
            layer.msg('请上传正确的身份证号码！');
            return false;
        }
        ...
        if (datasFlag == 1) {
            ...
            $.ajax({
                ...
                success: function(re) {
                    ...
                    layer.msg(re.msg);
                    setTimeout(function() {
                        $(".btn-navlist").children(".navbtn2").trigger("click");
                    }, 2000)
                },
                error: function(re) {
                    console.log(re);
    ...
    affirmResult: function(status) {
        ...
        var datas = {
            ...
            "shopId": Number(shopId)||Number($('#myShop').val()),
            "type": Number(type)
        };
        var data = JSON.stringify(datas);
...
function clearPage(){
    $(".nav_left").html('').hide();
    $(".apply_content").html('').hide();
    $(".show_apply").html('').hide();
    $(".apply_form").html('').hide();
    $('.apply_forms').html('').hide();
    $('.showapplyForm').html('').hide();
    $('#kongbai').remove();
}
function clearRight(){
    $(".apply_content").html('').hide();
...
function rightNoData(){
    $(".apply_content").html("<div id='kongbai'><img src='img/nodata.png'/></div>").show();
    $(".show_apply").html('').hide();
    $(".apply_form").html('').hide();
    $('.apply_forms').html('').hide();
    $('.showapplyForm').html('').hide();    
