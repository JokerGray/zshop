$(function(){
    ...
    function addHoverDom(treeId, treeNode){
        ...
        var optionBtn="<div class='option' id='diyBtn_" +treeNode.detailId+ "'><div></div>"
                      ...
                      +"<li class='editInfo' flag="+treeNode.detailId+">修改组信息</li>"
    ...
    function add(title,flag,name){
        ...
        layer.open({
            ...
            btn1:function(index, layero){
                var body = layer.getChildFrame('body', index);
                if(body.find('#teamName').val()==''||body.find('#person').val()=='选择领袖'||sessionStorage.getItem('src')==null||body.find('#detailTitle').val()==''){
                    layer.msg('请完善信息');
                    return;
                }
                let params={
                        userId:JSON.parse(body.find('#person').val()),
                        detailName:body.find('#teamName').val(),
                        detailTitle:body.find('#detailTitle').val(),
