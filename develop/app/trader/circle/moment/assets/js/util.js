...
function uploadOss(data) {
    var uploaderLabel = new plupload.Uploader({
        ...
        flash_swf_url: '../../assets/plupload-2.3.1/js/Moxie.swf',
        silverlight_xap_url: '../../assets/plupload-2.3.1/js/Moxie.xap',
...
function sendCallback(file, data) {
    ...
    setTimeout(function(){
        picArray.push({
            picWidth: $('#picPreview img').width(),
            picHeight: $('#picPreview img').height(),
            picUrl: urls
        })
        sessionStorage.setItem('picArray',JSON.stringify(picArray));
    },500)
...
function sendEditorVideoCallback(file, data) {
    ...
    $("#videoPreview").html($("<video src='" + videoUrls + "'></video>"));
    $('#addVideo').css('border','none');
    $('#cover').css('background',"url('"+videoUrls+"?x-oss-process=video/snapshot,t_0,m_fast') no-repeat");
    sessionStorage.setItem('videoUrl',videoUrls);
}
