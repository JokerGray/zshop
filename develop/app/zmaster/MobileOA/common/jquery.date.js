...
(function (factory) {
	...
	if(dpr == '2'){
		css = css.replace(/40px/g,'80px').replace('120px','240px');
	}else if(dpr == '3'){
		css = css.replace(/40px/g,'120px').replace('120px','360px');
	}
    ...
    createDate = {
        ...
        bissextile:function(year,month){
            ...
            $(doc).on(tmove,'#date-wrapper ol', function(e){
                ...
                if (that.position().top < -(that.height() - (120*dpr))) that.css('top', '-' + (that.height() - (120*dpr)) + 'px');
            })
            ...
            function dragEnd(that){
                ...
                that.css('top',Math.round(t/(40*dpr))*40*dpr+'px');
                //定位active
                t = Math.round(Math.abs($(that).position().top));
                var li = that.children('li').get(t/(40*dpr)+1);
                ...
                if(id == 'd-month' || id == 'd-year'){
                    ...
                    if(day != ($('#d-day li').length-2)){
                        ...
                        if(Math.abs($('#d-day').position().top) > $('#d-day').height()-120*dpr)$('#d-day').css('top','-'+($('#d-day').height()-120*dpr)+'px');
                    }
                }
            }
        },
        ...
        toNow:function(refresh){
            if (!refresh) {
                $('#date-wrapper ol').each(function(){
                    var that = $(this);
                    var liTop = -(that.children('.active').position().top -40*dpr);
                    that.animate({
                        top: liTop
                    },
                    600);
