...
function uploadOss(data) {
    var uploaderLabel = new plupload.Uploader({
        ...
        init: {
            PostInit: function() {
            	//微信内嵌浏览器兼容
            	$('#upload-left input').attr('accept','image/*')
            	$('#upload-right input').attr('accept','image/*')
            	$('#newUpload1 input').attr('accept','image/*')
            	$('#newUpload2 input').attr('accept','image/*')
...
function regCallback(file, data) {
    ...
    if(data.num==1){
    	console.log(urls);
    	sessionStorage.setItem('src1',urls);
    	var src1=sessionStorage.getItem('src1');
    	$('#upload-left').css({'background':'url('+src1+') no-repeat','background-size':'100% 100%'});
    }else if(data.num==2){
    	console.log(urls);
		sessionStorage.setItem('src2',urls);
		var src2=sessionStorage.getItem('src2');
		$('#upload-right').css({'background':'url('+src2+') no-repeat','background-size':'100% 100%'});
    }else if(data.num==3){
    	console.log(urls);
		sessionStorage.setItem('src3',urls);
		var src3=sessionStorage.getItem('src3');
		$('#newUpload1').css({'background':'url('+src3+') no-repeat','background-size':'100% 100%'});
    }else if(data.num==4){
    	console.log(urls);
		sessionStorage.setItem('src4',urls);
		var src4=sessionStorage.getItem('src4');
		$('#newUpload2').css({'background':'url('+src4+') no-repeat','background-size':'100% 100%'});
    }
    $('.preview div').css({'left':'0'})
