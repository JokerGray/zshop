...
flag:选择完成后回调标记
*/
function laydateInit(dom, string, min, max) {
...
//时间选择成功回调
function successful(dom, value) {
  var arr = value.split(' 至 ');
  if (!value) {
    $('#' + dom).attr('data-beginTime', '');
    $('#' + dom).attr('data-endTime', '');
  }
  $('#' + dom).attr('data-beginTime', arr[0]);
  $('#' + dom).attr('data-endTime', arr[1]);
}