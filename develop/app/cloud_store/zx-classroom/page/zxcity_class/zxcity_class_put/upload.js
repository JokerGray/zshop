...
function filter(data) {
    ...
    if (data.flag == "video") {
        obj = [{
            title: "files",
            extensions: "mpg,m4v,mp4,flv,3gp,mov,avi,rmvb,mkv,wmv,mp3,wma,ogg,rm,au,aac,flac,rm"
        }]
    } else {
        obj = [{
            title: "Image files",
...
function uploadOss(data) {
    var uploaderLabel = new plupload.Uploader({
        ...
        filters: {
            ...
            max_file_size: data.size || "1024mb", //最大只能上传10mb的文件
        ...
        init: {
            ...
            FilesAdded: function (up, files) {
                var file = up.getFile(0);
                var files = files[0];
                if (data.before) {
 
                    var b = data.before(up, files);
                    if (b == false) {
                        up.files.splice(0, 1);
                        return false;
                    }
            ...
            FileUploaded: function (up, file, info) {
                if (info.status == 200) {
                    ...
                    if (data.flag == "video") {
                        ...
                        return sendVideoCallback(file, data, data.btn);
...
//上传视频
function sendVideoCallback(file, data, e) {
    layer.msg("上传成功");
    var urls = uploadJson.host + "/" + get_uploaded_object_name(file.name);
    var name = get_uploaded_object_name(file.name);
    if (data.callback) {
        data.callback({
            urls: urls,
            name: name
