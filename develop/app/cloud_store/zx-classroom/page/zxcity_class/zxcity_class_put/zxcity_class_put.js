layui.use(['form', 'table', 'laypage', 'layer','element','laydate'], function () {
    ...
    function setLook(data){
        var online = '';
        if (data.online == 2) { //线下
            online = $('#online-content');//修改线上
        } else {
            online = $('#online-content');
        }
        //智享课堂 标题 
        online.find('.layui-form-selected').attr('disabled','disabled');
    ...
    function setLineData(data){
        ...
        line.find('#fenlei .cls-input').val(data.theme.name);
        line.find('#fenlei .cls-input').attr('data-id',data.theme.id);
    ...
    $('#go-back').on('click',function(){
        location.href = '../zxcity_class_list/zxcity_class_list.html?shopId='+shopId;
    })
...
//分类
// $('.row-content11').click(function(){
//     $(".row-content11 ul").slideDown(1000,function(){
//         $('html,body').animate({ scrollTop: document.getElementsByTagName('.row-content11 ul').scrollHeight}, 1000);
//         // layer.msg("扫描二维码报名");
//     //    return false;
//        });
// })
            ...
            if (res.code == 1) {
                ...
                if (res.data && res.data.length >= 1) {
                    ...
                    res.data.forEach(function (item, index) {
                        html += '<dd lay-value="' + item.id + '" class="">' + item.name;
                        html += '</dd>';
                    });
                    var dl = $('#fenlei dl');
                    dl.html(html);
...
//点击 下拉 选择分类
$('#fenlei #teacher-lists').on('click',function(){
    toggleFL();
    return false;
});
//选择切换分类
function toggleFL(){
    var is = $('#fenlei  .teacher-box i').hasClass('rotate');
    if (is) {
        $('#fenlei .teacher-box dl').hide();
        $('#fenlei  .teacher-box i').removeClass('rotate');
    } else {
        $('#fenlei .teacher-box dl').show();
        var length = $('#fenlei .teacher-box dl');
        if (!(length && length.length >= 1)) {
            $('#fenlei .teacher-box dl').html('<dd lay-value="0" class="">无</dd>');
        }
        $('#fenlei .teacher-box i').addClass('rotate');
    }
}
//点击 dd 设置 分类
$('#fenlei dl').on('click','dd',function(){
    var name = $(this).text();
    $('#fenlei .teacher-box input').val(name);
    $('#fenlei .teacher-box input').attr('data-id',$(this).attr('lay-value'));
    toggleFL();
    form.render();
});
$("#fenlei dl").blur(function () {
    $("#fenlei dl").hide();
});
...
// reqAjaxAsync(API.getThemes, JSON.stringify({})).
// done(function(res){
//     if(res.code == 1) {
//         // console.log("查询专栏信息");
//         // console.log(res);
//         // params.themeId=res.id || 1;
//         // params.themeId=res.name || 0;
//         // layer.msg('删除成功');
//         // li.remove();
//     }else {
//         layer.msg(res.msg);
...
// });
    ...
    //初始化 获取主讲人 列表
    function initTeacher(shopId){
        var param={
            shopId:shopId
        }
        //获取主讲人信息
        reqAjaxAsync(API.findTeacherList,JSON.stringify(param))
            .done(function (res) {
                if (res.code == 1) {
                    var html = '';
                    if (res.data && res.data.length >= 1) {
    ...
    function getUpdateParams(){
        ...
        //获取当前线上 线下
        if(type == 2) {
            line = $('#online-content');//修改为线上
        }else{
            line = $('#online-content');
        }
        //cover
        var params = {};
        // params.createId=80131;
       ...
       params.shopId=shopId;
        ...
        params.themeName=$('#fenlei .cls-input').val();
        var themeID=$('#fenlei .cls-input').attr('data-id');
        if(params.themeName == ''){
            layer.msg('请选择分类');
            return '';
        }
    ...
    //     var params = {};
    //     params.online = '2';
    //     params.type = $(offline.courseType).val();
    //     if(params.type == ''){
    //         layer.msg('请选择分类');
    //         return '';
    //     }
    ...
    //     params.createId = 571;//修改
    //     params.themeId=3;
    //     params.merchantId=1070;
    //     params.themeName="调试用1111111111111111111111111111111";
    //      //查询专栏信息
    //      reqAjaxAsync(API.getThemes, JSON.stringify({shopId:shopId})).
    //      done(function(res){
    //          if(res.code == 1) {
    //              console.log("查询专栏信息");
    //              console.log(res);
    //              params.themeId=res.id || "";
    //             params.themeName=res.name || "";
    //              // layer.msg('删除成功');
    //              // li.remove();
    //          }else {
    //              layer.msg(res.msg);
