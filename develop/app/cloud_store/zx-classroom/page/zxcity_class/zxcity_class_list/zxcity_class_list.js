layui.use(['form','table','laypage','layer'],function(){
    ...
    function resetInfo(){
        ...
        //pageCallback回调
    function pageCallback(index, limit, keyword,shopId) {
        var shopId = GetQueryString('shopId');
        // var shopId=288;
        if (keyword == undefined) { keyword = '' }
        var keyword = keyword || $('#search_box').val();
        return getData(USER_API.findList, "{'pageNo':" + index + ",'pageSize':" + limit + ",'keyword':'" + keyword + "','shopId':'"+shopId+"'}");
    }
    //切换，查询云店智享课堂
    ...
    function chaxun(Id){
        ...
        reqAjaxAsync('shop/getShopByID', JSON.stringify(param)).done(function (res){
            if(res.code==1){
                    $('input[name=sex]').each(function(i,item){
                        if($(item).val()==res.data.masterSetupType){
    ...
    form.on('radio(gis)', function(){
            ...
            layer.open({
                ...
                closeBtn: 1,
                anim: 5,
                content: ({icon: 2}, '确认修改云店课堂开启，或关闭的状态吗?'),
                btn: ['确定', '取消'],
                btnAlign: 'r',
                ...
                reqAjaxAsync('shop/updateMasterSetup', JSON.stringify(param)).done(function (res){
                    if(res.code==1){
                     layer.msg(res.msg);
                    }else{
                        layer.msg(res.msg);
    ...
    $('#add-course').on('click',function(){
        location.href = '../zxcity_class_put/zxcity_class_put.html?shopId='+shopId;
    });
    ...
    table.on('tool(table)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
        ...
        if (layEvent === 'change') {
            ...
            var shopId = GetQueryString('shopId');
            location.href = '../zxcity_class_put/zxcity_class_put.html?courseId=' + data.id+"&shopId="+shopId;
            ...
            sessionStorage.setItem("courseId", data.id);
            location.href = '../zxcity_class_put/zxcity_class_put.html?courseId=' + data.id + "&look=true"+"&shopId"+shopId;
        }
    });
