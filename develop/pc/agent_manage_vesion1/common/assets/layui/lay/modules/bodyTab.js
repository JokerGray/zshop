...
layui.define(["element", "jquery"], function(exports) {
	...
	Tab.prototype.render = function() {
		...
		reqAjaxAsync('operations/getFomatAgentManagerMenu').then(function(res) {
			//显示左侧菜单
			if($(".navBar").html() == '') {
				var _this = this;
	...
	Tab.prototype.tabMove = function() {
		$(window).on("resize", function() {
			...
			if(topTabsWidth > topTabsBoxWidth) {
				...
				//pc端拖动效果
				topTabs.on("mousedown", godown);
				topTabs.on("mousemove", gomove);
				$(document).on("mouseup", goend);
				//移动端拖动效果
				topTabs.on("touchstart", godown);
				topTabs.on("touchmove", move);
				topTabs.on("touchend", end);
			} else {
				//移除pc端拖动效果
				topTabs.off("mousedown", godown);
				topTabs.off("mousemove", gomove);
				topTabs.off("mouseup", goend);
				//移除移动端拖动效果
				topTabs.off("touchstart", godown);
				topTabs.off("touchmove", gomove);
				topTabs.off("touchend", goend);
