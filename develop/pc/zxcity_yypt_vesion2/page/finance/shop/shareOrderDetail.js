layui.use(['form', 'layer', 'jquery', 'laydate', 'table', 'laypage'], function() {
			...
			//接口
			var server = {
				dataList: 'payTransactionLog/shareOrderDetail', //(查询列表)
				exportExcel: 'payExcel/exportShareOrderDetail'//(下载地址)
			}
			...
			function pageCallback(index, limit, callback) {
				...
				reqAjaxAsync(server.dataList, JSON.stringify(parms)).then(function(res) {
					...
					$.each(data, function(i, item) {
						$(item).attr('eq', (i + 1));
						//格式化金额
						$(item).attr('originalPrice', fmoney(item.originalPrice, 2));
						$(item).attr('cashback', fmoney(item.cashback, 2));
						$(item).attr('orderPrice', fmoney(item.orderPrice, 2));
