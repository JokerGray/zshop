layui.use(['form', 'layer', 'jquery', 'laydate', 'table', 'laypage'], function() {
			...
			//接口
			var server = {
				dataList: 'payCoupon/getfinanceCheckResult', //(查询列表)
				exportPayScInvoiceLog : 'payCoupon/exportfinanceCheckResult'//(下载地址)
			}
			...
			$('#addButton').on('click', function(){
					var parms = {					
							start: $('.startDate').eq(0).attr('data-date') || "",
							end: $('.endDate').eq(0).attr('data-date') || ""													
					};
				...
				function pageCallback(index, limit, callback) {
					...
					reqAjaxAsync(server.dataList, JSON.stringify(parms)).then(function(res) {
						...
						$.each(data, function(i, item) {
							$(item).attr('eq', (i + 1));
							$(item).attr('merchantBalance',fmoney(item.merchantBalance,2));
							$(item).attr('userBalance',fmoney(item.userBalance,2));
							$(item).attr('uncompleteBalance',fmoney(item.uncompleteBalance,2));
							$(item).attr('income',fmoney(item.income,2));
							$(item).attr('recharge',fmoney(item.recharge,2));
							$(item).attr('withdraw',fmoney(item.withdraw,2));
							$(item).attr('incomeTypeOne',fmoney(item.incomeTypeOne,2));
						});
						return callback(res);
					})
				}
