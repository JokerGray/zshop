layui.use(['form', 'layer', 'jquery', 'laydate', 'table', 'laypage'], function() {
			...
			//接口
			var server = {
				dataList: 'profitExtractLog/selLaborTaxDetail', //(查询列表)
				exportExcel: 'scLaborTaxList/exportScLaborTaxDetail'//(下载地址)
			}
				...
				function pageCallback(index, limit, callback) {
					...
					reqAjaxAsync(server.dataList, JSON.stringify(parms)).then(function(res) {
						...
						$.each(data, function(i, item) {
							...
							$(item).attr('laborTax',fmoney(item.laborTax, 2))
							$(item).attr('afterTaxAmount',fmoney(item.afterTaxAmount, 2))
							...
							if(item.laborCostsMonthlyTotal==null){
								$(item).attr('laborCostsMonthlyTotal','-')
							};
							if(item.laborTaxMonthlyTotal==null){
								$(item).attr('laborTaxMonthlyTotal','-')
							};							
