layui.use(['form', 'layer', 'jquery', 'laydate', 'table', 'laypage'], function() {
			...
			//接口
			var server = {
				dataList: 'payScInvoiceLog/monthlyInvoiceReportList', //(查询列表)
				exportPayScInvoiceLog : 'payScInvoiceLog/exportMonthlyInvoiceReportList'//(下载地址)
			}
				...
				function pageCallback(index, limit, callback) {
					...
					reqAjaxAsync(server.dataList, JSON.stringify(parms)).then(function(res) {
						...
						$.each(data, function(i, item) {
							$(item).attr('eq', (i + 1));
							$(item).attr('money1',fmoney(item.sumAmount),2);
							$(item).attr('money2',fmoney(item.sumInvoiceAmount),2);
						});
						return callback(res);
					})
				}
				...
				function tableInit() {
					var _obj = _tableInit('merchantTable', [
							[{
								...
								title: '企业数量',
								align: 'center',
								field: 'countInvoiceCompany'
							}, {
								title: '发票数量',
								align: 'center',
								field: 'countInvoiceNo'
							}, {
								title: '申报总金额',
								align: 'center',
								field: 'money1'
