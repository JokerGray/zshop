layui.use(['form', 'layer', 'jquery', 'laydate', 'table', 'laypage'], function() {
			...
			//接口
			var server = {
				select: 'payScInvoiceLog/select', //(查询列表)
				selectModuleList: 'scAlipayPoundage/selectModuleList', //(模块列表)
				exportPayScInvoiceLog : 'payScInvoiceLog/exportPayScInvoiceLog'//(下载地址)
			}
			...
			$('body').on('click', '.layui-layer .layui-layer-content .layer-place', function() {
				...
				function pageCallback(index, limit, callback) {
					...
					reqAjaxAsync(server.select, JSON.stringify(parms)).then(function(res) {
						if(res.code != 1) {
						...
						$.each(data, function(i, item) {
							$(item).attr('eq', (i + 1));
							$(item).attr('money1',fmoney(item.amount,2));
							$(item).attr('money2',fmoney(item.invoiceAmount,2));
						});
						return callback(res);
					})
				}
