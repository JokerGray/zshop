layui.use(['form', 'layer', 'jquery', 'laydate', 'table', 'laypage'], function() {
	...
	//接口
	var server = {
		selectList: 'scLaborCostsExtractionRecords/selectList', //(查询列表)
		exportLaborCostsExtractionRecords: 'scLaborCostsExtractionRecords/exportLaborCostsExtractionRecords', //(下载地址)
	}
	...
	$('#addButton').on('click', function() {
		...
		downloadFile(server.exportLaborCostsExtractionRecords, JSON.stringify(parms))
	})
	...
	function pageCallback(index, limit, callback) {
		...
		reqAjaxAsync(server.selectList, JSON.stringify(parms)).then(function(res) {
			...
			$.each(data, function(i, item) {
				$(item).attr('eq', (i + 1));
				$(item).attr('laborCosts',fmoney(item.laborCosts,2));
				$(item).attr('extractTheAmount',fmoney(item.extractTheAmount,2));
			});
			return callback(res);
		})
	}
	...
	function tableInit() {
		var _obj = _tableInit('merchantTable', [
				[{
					...
					title: '提取金额',
					align: 'left',
					field: 'extractTheAmount'
				}, {
					title: '提取人',
					align: 'left',
					field: 'extractPersonName'
				}, {
					title: '提取时间',
					align: 'left',
