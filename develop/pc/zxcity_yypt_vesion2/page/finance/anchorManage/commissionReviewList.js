layui.use(['form', 'layer', 'jquery', 'laydate', 'table', 'laypage'], function() {
			...
			//接口
			var server = {
				dataList: 'anchorManager/commissionReviewRecordList', //(查询列表)
				selectModuleList: 'scAlipayPoundage/selectModuleList', //(模块列表)
				exportPayScInvoiceLog : 'anchorManager/commissionReviewRecordListExport'//(下载地址)
			}
					...
					reqAjaxAsync(server.dataList, JSON.stringify(parms)).then(function(res) {
						...
						$.each(data, function(i, item) {
							...
							if(item.status==0){
								...
								$(item).attr('status','未通过')
							};
							if(item.name==null){
								$(item).attr('name','-')
							};
							if(item.afterTaxAmount==null){
								$(item).attr('afterTaxAmount','-')
							};
							if(item.laborTax==null){
								$(item).attr('laborTax','-')
							}
						});
						return callback(res);
			...
			table.on('tool(merchantTable)', function(obj) {
				...
				if(obj.event === 'mark') {
						...
						success: function(index,layero) {
								$('#describe').val(data.reviewRemark)
