layui.use(['form', 'layer', 'jquery', 'laydate', 'table', 'laypage'], function() {
			...
			//接口
			var server = {
				dataList: 'payStatistics/selectAllMerchantAccountBalance', //(查询列表)
				exportPayScInvoiceLog : 'payExcel/exportAllMerchantAccountBalance'//(下载地址)
			}
			...
			$('#addButton').on('click', function(){
					var parms = {
							dataTable:{
								userType:$('#accountType').val()||"",
								accountName: $('#accountName').val()||"",
								createTimeStart: $('.startDate').eq(0).attr('data-date') || "",
								createTimeEnd: $('.endDate').eq(0).attr('data-date') || ""									
							}
				...
				function pageCallback(index, limit, callback) {
					...
					reqAjaxAsync(server.dataList, JSON.stringify(parms)).then(function(res) {
						...
						$.each(data, function(i, item) {
							$(item).attr('eq', (i + 1));
							$(item).attr('money',fmoney(item.accountBalacne,2));
							if(item.merchantType==0){
								$(item).attr('merchantType','正式商户')
							}else if(item.merchantType==1){
								$(item).attr('merchantType','试用商户')
							}
						});
						return callback(res);
					})
