layui.use(['form', 'layer', 'jquery', 'laydate', 'table', 'laypage'], function() {
			...
			//接口
			var server = {
				dataList: 'payStatistics/selUnsuccessfulBusiness', //(查询列表)
				exportPayScInvoiceLog : 'payExcel/exportUnsuccessfulBusiness',//(下载地址)
				detial:'payStatistics/selUnsuccessfulBusinessInfo'//详情
			}
			...
			table.on('tool(merchantTable)', function(obj) {
				...
				if(obj.event === 'detial') {
						...
						success: function(index,layero) {
							reqAjaxAsync(server.detial,JSON.stringify(pram)).then(function(res){
								var resData=res.data;
								$('#serialNo').val(resData.serialNo);
								$('#relationNo').val(resData.relationNo);
								$('#payerNo').val(resData.payerNo);
								$('#payerName').val(resData.payerName);
								$('#billTime').val(resData.billTime);
								$('#money').val(resData.money);
								$('#payeeNo').val(resData.payeeNo);
								$('#payeeName').val(resData.payeeName);
