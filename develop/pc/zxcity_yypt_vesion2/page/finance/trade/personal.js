layui.use(['form', 'layer', 'jquery', 'laydate', 'table', 'laypage'], function() {
			...
			//接口
			var server = {
				dataList: 'payTransactionLog/transactionDetail', //(查询列表)
				exportPayScInvoiceLog : 'payExcel/exportTransactionDetail',//(下载地址)
			}
				...
				function pageCallback(index, limit, callback) {
					...
					reqAjaxAsync(server.dataList, JSON.stringify(parms)).then(function(res) {
						...
						$.each(data, function(i, item) {
							$(item).attr('eq', (i + 1));
							$(item).attr('money',fmoney(item.money,2));
							if(item.relationNo==""){
								$(item).attr('relationNo','-')
							};
							if(item.remark==""){
								$(item).attr('remark','-')
							}
