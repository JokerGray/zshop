layui.use(['form', 'layer', 'jquery', 'laydate', 'table', 'laypage'], function() {
			...
			//接口
			var server = {
				dataList: 'merchantUpdate/employeeCommissionList', //(查询列表)
				exportExcel: 'merchantUpdate/employeeCommissionListExport'//(下载地址)
			}
			// 金额格式化
			function fmoney(s, n) {
				...
				function pageCallback(index, limit, callback) {
					...
					reqAjaxAsync(server.dataList, JSON.stringify(parms)).then(function(res) {
						...
						$.each(data, function(i, item) {
							$(item).attr('eq', (i + 1));
							$(item).attr('money',fmoney(item.laboramount,2))
						});
						return callback(res);
					})
				}
				...
				function tableInit() {
					var _obj = _tableInit('merchantTable', [
							[{
								...
								title: '线下合作商户数/3650.00',
								align: 'left',
								field: 'offlinePartnerLog'
							}, {
								title: '线下代理商户数/10万',
								align: 'center',
								field: 'offlineAgentLog'
							}, {
								title: '线上合作商户数/3650.00',
								align: 'center',
								field: 'onlinePartnerLog'
							}, {
								title: '线上代理商户数/10万',
								align: 'left',
								field: 'onlineAgentLog'
							}, {
								title: '劳务所得总额',
								align: 'left',
								field: 'money'
