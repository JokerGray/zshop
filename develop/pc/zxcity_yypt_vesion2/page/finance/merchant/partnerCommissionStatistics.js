layui.use(['form', 'layer', 'jquery', 'laydate', 'table', 'laypage'], function() {
			...
			//接口
			var server = {
				dataList: 'merchantUpdate/partnerCommissionList', //(查询列表)
				exportPayScInvoiceLog : 'merchantUpdate/partnerCommissionListExport'//(下载地址)
			}
				...
				function pageCallback(index, limit, callback) {
					...
					reqAjaxAsync(server.dataList, JSON.stringify(parms)).then(function(res) {
						...
						$.each(data, function(i, item) {
							$(item).attr('eq', (i + 1));
							if(item.partnerCount==null){
								$(item).attr('partnerCount','-');
							};
							if(item.agentCount==null){
								$(item).attr('agentCount','-');
							};
							if(item.copartnerPartnerCount==null){
								$(item).attr('copartnerPartnerCount','-');
							};
							if(item.copartnerAgentCount==null){
								$(item).attr('copartnerAgentCount','-')
							}
						});
						return callback(res);
					})
				...
				function tableInit() {
					var _obj = _tableInit('merchantTable', [
							[{
								...
								title: '下线办理合作商户数',
								align: 'center',
								field: 'copartnerPartnerCount'
							}, {
								title: '下线办理代理商户数',
								align: 'center',
								field: 'copartnerAgentCount'
							}, {
								title: '劳务所得总额',
								align: 'center',
								field: 'laborCost'
