layui.use(['form', 'layer', 'jquery', 'laydate', 'table', 'laypage'], function() {
			...
			//接口
			var server = {
				dataList: 'merchantUpdate/recommendCommissionList', //(查询列表)
				exportPayScInvoiceLog : 'merchantUpdate/recommendCommissionListExport'//(下载地址)
			}
				...
				function pageCallback(index, limit, callback) {
					...
					reqAjaxAsync(server.dataList, JSON.stringify(parms)).then(function(res) {
						...
						$.each(data, function(i, item) {
							$(item).attr('eq', (i + 1));
							$(item).attr('money1',fmoney(item.laboramount,2))
							$(item).attr('money2',fmoney(item.labortax,2))
						});
						return callback(res);
					})
				}
