layui.use(['form', 'layer', 'jquery', 'laydate', 'table', 'laypage'], function() {
		...
		//接口
		var server = {
			dataList: 'profitExtractLog/selMerchantUpgradeProfitDetail', //(查询列表)
			exportExcel: 'payExcel/exportMerchantUpgradeProfitDetail'//(下载地址)
		}
		...
		$('#addButton').on('click', function(){
	        ...
	        if(startTimes =="" && endTimes == ""){
	          parms = "{'commissionStatus':"+commissionStatus+",'onOffLine':"+onOffLine+"}";
	        }else if(startTimes !=="" && endTimes ==""){
	          parms = "{'referenceAuditTimeStart':'"+startTimes+' 00:00:00'+"','commissionStatus':"+commissionStatus+",'onOffLine':"+onOffLine+"}";
	        }else if(startTimes =="" && endTimes !==""){
	          parms = "{'referenceAuditTimeEnd':'"+endTimes+' 23:59:59'+"','commissionStatus':"+commissionStatus+",'onOffLine':"+onOffLine+"}";
	        }else if(startTimes !=="" && endTimes !==""){
	          parms = "{'referenceAuditTimeStart':'"+startTimes+' 00:00:00'+"','referenceAuditTimeEnd':'"+endTimes+' 23:59:59'+"','commissionStatus':"+commissionStatus+",'onOffLine':"+onOffLine+"}";
	        }
...
//						pageSize: limit,
//						pageNo: index,
//						extractType: $('#accountType').val() || "",
//						extractTimeStart: $('.startDate').eq(0).attr('data-date') || "",
//						extractTimeEnd: $('.endDate').eq(0).attr('data-date') || ""
//					};
				var parms="";
				var pageSize=limit;
				var pageNo=index;
				...
				var	endTimes= $('#endDate').val();
		        if(startTimes =="" && endTimes == ""){
		          parms = "{'pageNo':"+pageNo+",'pageSize':"+pageSize+",'commissionStatus':"+commissionStatus+",'onOffLine':"+onOffLine+"}";
		        }else if(startTimes !=="" && endTimes ==""){
		          parms = "{'referenceAuditTimeStart':'"+startTimes+' 00:00:00'+"','pageNo':"+pageNo+",'pageSize':"+pageSize+",'commissionStatus':"+commissionStatus+",'onOffLine':"+onOffLine+"}";
		        }else if(startTimes =="" && endTimes !==""){
		          parms = "{'referenceAuditTimeEnd':'"+endTimes+' 23:59:59'+"','pageNo':"+pageNo+",'pageSize':"+pageSize+",'commissionStatus':"+commissionStatus+",'onOffLine':"+onOffLine+"}";
		        }else if(startTimes !=="" && endTimes !==""){
		          parms = "{'referenceAuditTimeStart':'"+startTimes+' 00:00:00'+"','referenceAuditTimeEnd':'"+endTimes+' 23:59:59'+"','pageNo':"+pageNo+",'pageSize':"+pageSize+",'commissionStatus':"+commissionStatus+",'onOffLine':"+onOffLine+"}";
		        }					
				reqAjaxAsync(server.dataList, parms).then(function(res) {
					...
					$.each(data, function(i, item) {
						...
						if(item.merchantPhone==null){
							$(item).attr('merchantPhone','-')
						};
						if(item.referenceAuditTime==null){
							$(item).attr('referenceAuditTime','-')
						}
					});
					return callback(res);
				})
