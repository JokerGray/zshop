layui.use(['form', 'layer', 'jquery', 'laydate', 'table', 'laypage'], function() {
		...
		function getSearchParam(pageNo, pageSize){
		    ...
		    var homeQueryType = getRequestAddressUrlParameter('homeQueryType');
		...
		function pageCallback(index, limit, callback) {
			...
			reqAjaxAsync(server.dataList, parms).then(function(res) {
				...
				$.each(data, function(i, item) {
					...
					//到账金额
					var upgradeRealAmount = item.upgradeRealAmount;
					if(upgradeRealAmount){
						val('upgradeRealAmount', fmoney(upgradeRealAmount, 2));
					}
					//盈利金额
					var profitAmount = item.profitAmount;
					if(profitAmount){
						val('profitAmount', fmoney(profitAmount, 2));
					}
					//升级类型
					var upgradeTypeStr = "";
                    if(item.onOffLine==1){
                	...
                	//推荐人佣金
                    var referenceRealAmount = item.referenceRealAmount;
                    if(referenceRealAmount){
                    	val('referenceRealAmount', fmoney(referenceRealAmount, 2));
                    }
                    //合伙人佣金
                    var copartnerAmount = item.copartnerAmount;
                    if(copartnerAmount){
                    	val('copartnerAmount', fmoney(copartnerAmount, 2));
                    }
					//状态
