layui.use(['form', 'layer', 'jquery', 'laydate', 'table', 'laypage'], function() {
			...
			function getSearchParam(pageNo, pageSize){
			    ...
			    var commissionStatus = getRequestAddressUrlParameter('commissionStatus');
			    var employeeCommissionStatus= getRequestAddressUrlParameter('employeeCommissionStatus');
			...
			function pageCallback(index, limit, callback) {
				...
				reqAjaxAsync(server.dataList, parms).then(function(res) {
					...
					$.each(data, function(i, item) {
						...
						//间隔天数
                		var upgradeAutitTime = item.upgradeAutitTime;
                		if(upgradeAutitTime){
                			var date = new Date(upgradeAutitTime);
                		    ...
                		    // 当前时间减去 过去时间
                		    var interval = parseInt(intervalTime/(24*60*60*1000))+"天";
                		    val('upgradeAutitTime', interval);
                		}else{
                			val('upgradeAutitTime', "-");
                		}
					});
					return callback(res);
				})
