layui.use(['form', 'layer', 'jquery', 'laydate', 'table', 'laypage'], function() {
			...
			//接口
			var server = {
				dataList: 'payTransactionLog/selMerchantStatistics', //(查询列表)
				exportPayScInvoiceLog : 'payExcel/exportMerchantStatistics'//(下载地址)
			}
				...
				function pageCallback(index, limit, callback) {
					...
					reqAjaxAsync(server.dataList, JSON.stringify(parms)).then(function(res) {
						...
						$.each(data, function(i, item) {
							$(item).attr('eq', (i + 1));
							$(item).attr('money1',fmoney(item.amountRecharge,2));
							$(item).attr('money2',fmoney(item.amountSales,2));
							$(item).attr('money3',fmoney(item.ab,2));
						});
						return callback(res);
					})
				}
...
//				  	   			xData.push(i);
//				  	   		};
			           		var option = echartsCustomerFunction.initOption('商户名称', 
			           				  					['充值次数','充值金额','销售次数','销售金额','账户余额'],
			           				  				yData['accountName'], 
			           				  				[yData['totalRecharge'],yData['amountRecharge'],yData['totalSales'],yData['amountSales'],yData['ab']]);
	...
	//							
							...
							success: function(index,layero) {
								...
								reqAjaxAsync('payStatistics/selectUserInfo',JSON.stringify(parms)).then(function(res){
									var data=res.data;									
									$('#userCode').val(data.userCode);
									$('#userName').val(data.userName);
									$('#regular').val(data.regular);
									$('#locked').val(data.locked);
									$('#platformUserCode').val(data.platformUserCode);
									$('#registType').val(data.registType);
