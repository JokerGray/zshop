layui.use(['form', 'layer', 'jquery', 'laydate', 'table', 'laypage'], function() {
			...
			//接口
			var server = {
				dataList: 'payTransactionLog/selWithdrawCashSummary', //(查询列表)
				exportPayScInvoiceLog : 'payExcel/exportWithdrawCashSummary'//(下载地址)
			}
			...
			$('#addButton').on('click', function(){
					var parms = {					
					        type: 'month',
							ismerchant: $('#type').val()||"",
							returnTimeStart: $('.startDate').eq(0).attr('data-date') || "",
							returnTimeEnd: $('.endDate').eq(0).attr('data-date') || ""												
					};
					...
					reqAjaxAsync(server.dataList, JSON.stringify(parms)).then(function(res) {
						...
						$.each(data, function(i, item) {
							...
							if(item.ismerchant==0){
								$(item).attr('type','平台用户')
							}else if(item.ismerchant==1){
								$(item).attr('type','商户')
							}
							$(item).attr('money1',fmoney(item.money,2));
						});
						return callback(res);
					})
				}
