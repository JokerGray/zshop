layui.use(['form', 'layer', 'jquery', 'laydate', 'table', 'laypage'], function() {
			...
			//接口
			var server = {
				dataList: 'payTransactionLog/selWithdrawCashApplySummary', //(查询列表)
				exportPayScInvoiceLog : 'payExcel/exportWithdrawCashApplySummary'//(下载地址)
			}
				...
				function pageCallback(index, limit, callback) {
					...
					reqAjaxAsync(server.dataList, JSON.stringify(parms)).then(function(res) {
						...
						$.each(data, function(i, item) {
							...
							$(item).attr('money1',fmoney(item.applyMoney,2));
							$(item).attr('money2',fmoney(item.payMoney,2));
							$(item).attr('money3',fmoney(item.accountBalance,2));
						});
						return callback(res);
					})
				}
