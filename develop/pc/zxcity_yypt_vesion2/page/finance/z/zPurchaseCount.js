layui.use(['form', 'layer', 'jquery', 'laydate', 'table', 'laypage'], function() {
			...
			//接口
			var server = {
				dataList: 'payTransactionLog/selPurchaseStatistics', //(查询列表)
				exportPayScInvoiceLog : 'payExcel/exportPurchaseStatistics'//(下载地址)
			}
				...
				function pageCallback(index, limit, callback) {
					...
					reqAjaxAsync(server.dataList, JSON.stringify(parms)).then(function(res) {
						...
						$.each(data, function(i, item) {
							$(item).attr('eq', (i + 1));
							if(item.regModeStr==null){
								$(item).attr('regModeStr','-')
							};

							$(item).attr('money1',fmoney(item.rechargeMoney,2));
							$(item).attr('money2',fmoney(item.ab,2));
							if(item.ab==null){
								$(item).attr('money2','-')
							}							
						});
					...
					tableIns = table.render({
						...
						cellMinWidth:80,
							...
							$('body').on('click', '.layui-table-body table tr', function() {
			           		...
			           		var option = echartsCustomerFunction.initOption('智币购买统计', 
			           				  					['购买次数','购买金额','智币余额'],
			           				  				xData, 
			           				  				[yData['number'],yData['rechargeMoney'],yData['ab']]);
