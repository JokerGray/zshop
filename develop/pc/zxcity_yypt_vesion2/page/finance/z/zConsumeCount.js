layui.use(['form', 'layer', 'jquery', 'laydate', 'table', 'laypage'], function() {
			...
			//接口
			var server = {
				dataList: 'payTransactionLog/selConsumeStatistics', //(查询列表)
				exportPayScInvoiceLog : 'payExcel/exportConsumeStatistics'//(下载地址)
			}
				...
				function pageCallback(index, limit, callback) {
					...
					reqAjaxAsync(server.dataList, JSON.stringify(parms)).then(function(res) {
						...
						$.each(data, function(i, item) {
							$(item).attr('eq', (i + 1));
							if(item.regMode==null){
								$(item).attr('regMode','-');
							};
							$(item).attr('money1',fmoney(item.subTotalSum,2));
					...
					tableIns = table.render({
						...
						cellMinWidth:80,
							...
							$('body').on('click', '.layui-table-body table tr', function() {
			           		...
			           		var option = echartsCustomerFunction.initOption('智币消耗统计', 
			           				  					['消耗次数','消耗金额','智币余额'],
			           				  				xData, 
			           				  				[yData['number'],yData['subTotalSum'],yData['ab']]);
