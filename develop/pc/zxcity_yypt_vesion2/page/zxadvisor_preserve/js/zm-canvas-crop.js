...
ZmCanvasCrop.prototype = {
	...
	drawCavDown: function(src){
	    ...
	    self._img.onload = function(){
	    	...
	    	if(!self.imgTrue()){
				layer.msg('图片大小必须大于:' + self._option.crop_min_width + '*' + self._option.crop_min_height);
	    		return;
	    	} 
	    	//让宽或者高撑满
	    	self.setShowImg();
...
/*					$('.resize-point').off('mousedown');
	...
	save: function(){
		...
		var self = this;
		var $result = $("<canvas width='" + self._option.crop_box_width + "' height='" + self._option.crop_box_height + "'></canvas>");
		/*var $show = $("<canvas width='" + self._option.crop_box_width + "' height='" + self._option.crop_box_width + "'></canvas>");*/
		$('.canvas-show1').html($result);//保存后实际大小存入
		/*$('.canvas-show').html($show);//保存后展示区域*/
		$result[0].getContext('2d').drawImage(self._img, self._save.left, self._save.top, self._save.width, self._save.height, 0, 0, self._option.crop_box_width, self._option.crop_box_height);
		/*$show[0].getContext('2d').drawImage(self._img, self._save.left, self._save.top, self._save.width, self._save.height, 0, 0, self._option.crop_box_width, self._option.crop_box_height);*/
		var base64Url = $result[0].toDataURL('image/jpeg');

		$('#imageId').val(base64Url);
		$(".canvas-show1").attr("src",base64Url);
		var realData = base64Url.replace('data:image/jpeg;base64,', '');
		$('#val').val(realData);

	//	saveCallBack && saveCallBack(base64Url, self._option.crop_box_width, self._option.crop_box_height);
		return [base64Url, self._option.crop_box_width, self._option.crop_box_height];
	},
