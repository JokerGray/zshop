...
layui.use(['form', 'layer', 'jquery', 'laydate', 'table', 'laypage'], function () { 
                ...
                success: function (layero, index) {
                    ...
                    if (content && content.indexOf) {
                        ...
                        ary.forEach(function(item,index){
                            ...
                            var list = $("#select-boxs .packagelist").eq(index);
                            var val = item.substr(0,1);
                            var tmpData = item.substr(2);
  
                            var coupon = list.find('select[name="coupon"]');
                            list.find(".packagecard .selectcoupon").html(g_couponType);
                            list.find('select[name="changbasic"]').val(val);
                            if(val == '1'){
                                coupon.val(tmpData);
                            }else if(val == '2'){
                                list.find('.packagecard').hide();
                                list.find('.packagecoin').show();
                                list.find('input[name="coin"]').val(tmpData);
                            }else if(val == '3'){
                                list.find('input[name="money"]').val(tmpData);
                                list.find('.packagecard').hide();
                                list.find('.packagemoney').show();
                            }
 
                        });
                        // couponType($("#select-boxs"), tmpData);
                        form.render();
                    }
                    //删除
    ...
    //优惠券类型
    function couponType(event, tmpData){
            var opHtml = "";
            ...
            if(res.code == 1){
                ...
                // tmpData && event.find(".packagecard .selectcoupon").val(tmpData);
                form.render();
            }else{
                layer.msg(res.msg);
            }
    ...
    $("#select-boxs").on("click", ".packagelist .deletebtn i", function () {
        var parent = $(this).parents('.packagelist');
        parent.remove();
    });
    ...
    function changSele(e){
        ...
        var package = e.parents('.packagelist');
        if(val == 1){
            // e.next().show();
            // e.parent().find(".packagecoin").hide();
            // e.parent().find(".packagemoney").hide();
            package.find('.packagecard').show();
            package.find('.packagecoin').hide();
            package.find('.packagemoney').hide();

            couponType($("#select-boxs"));
        }else if(val == 2){
            package.find('.packagecard').hide();
            package.find('.packagecoin').show();
            package.find('.packagemoney').hide();
        }else if(val == 3){
            package.find('.packagecard').hide();
            package.find('.packagecoin').hide();
            package.find('.packagemoney').show();
        }
        form.render();
    }
    //添加
    var packagecont = '<div class="layui-form-item packagelist">'+
    ...
    var packageconts = '<div class="layui-form-item packagelist">' +
        '<div class="layui-inline" >' +
        '<select name="changbasic" lay-verify="changbasic" class="selectpicker show-tick">' +
        '<option value="1">优惠券</option>' +
        '<option value="2">智币</option>' +
