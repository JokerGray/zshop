...
function initUpload(data) {
  ...
  uploader.bind('FilesAdded', function (up, files) {
    plupload.each(files, function (file) {
      //公告添加附件回调
      if (data.type == '1') {
        return announcementStart(up, file);
      }//审批上传附件回调
      else if (data.type == '2') {
...
function announcementStart(up, file) {
  ...
  //动态添加
  $('#details-1 .layui-form-item:eq(6) .layui-input-block').html(
    '<div class="annex" data-id="' + file.id + '">' +
    '<i class="wps"></i>' +
    '<p class="txt">' + file.name + '</p>' + plupload.formatSize(file.size) + '<i class="layui-icon">&#x1006;</i>' +
    '</div>');
}

//公告附件上传完成回调
function announcementEnd(src) {
  $('#details-1 .annex').attr('data-fileUrl', src);
}
...
function approvalStart(up, file) {
  up.randomName = randomName();
  //动态添加
  $('#iptFile').html('<div class="fileShow" data-id="' + file.id + '"><img src="../images/z.png" alt=""><span class="name">' + file.name + '</span><a>' + plupload.formatSize(file.size) + '</a><i class=" delFile layui-icon">&#x1006;</i></div>')
}

//审批附件上传完成回调
function approvalEnd(src) {
  $('#iptFile').attr('data-fileUrl', src);
}
...
function picStart(up) {
  ...
  $("#iptPic").find("div.parentPic").each(function (i,v) {
...
//审批图片上传完成回调
function picEnd(src) {
  $('#iptPic').css("height", "90px").append('<div class="parentPic"><i class="layui-icon toDelPic">&#x1007;</i><img src="' + src + '"  class="layui-upload-img"></div>')
  $('.parentPic').attr('data-fileUrl', src);
}
