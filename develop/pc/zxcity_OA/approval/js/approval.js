(function ($) {
    ...
    if (index == 0) {
      ...
      laypage.render({
        ...
        jump: function (obj, first) {
          ...
          if (!first) {
            var res1 = reqAjax(REQUEST_URL.SELECTSCLEAVEPAGE, JSON.stringify(param));
            init4(res1);
          }
        }
      });
  ...
  $('#approval .title button').on('click', function () {
    layer.open({
      ...
      yes: function (index, layero) {
        ...
        var reason = layedit.getText(curr);
        console.log(reason);
        var param = {
          userId: userId,
          /*shopId: shopId,*/
        ...
        var res = reqAjax(REQUEST_URL.SAVEORUPDATESCLEAVE, JSON.stringify(param));
        if (res.code == 1) {
      ...
      btn2: function (index, layero) {
        ...
        var res = reqAjax(REQUEST_URL.STARTLEAVEPROCESS, JSON.stringify(param));
        if (res.code == 1) {
          top.layer.msg(res.msg);
          setTimeout(function () {
            layer.close(index);
  ...
  //草稿查看详情弹窗
  $('#approval .item-list').on('click', '.item[data-approvalStatus=1] .item-info,.item[data-approvalStatus=1] .detail', function () {
    ...
    layer.open({
      ...
      btn2: function (index, layero) {
        ...
        layer.confirm('确定要删除此审批吗?', {title: '提示'}, function (index) {
          var res = reqAjax(REQUEST_URL.DELETESCLEAVE, JSON.stringify(param));
          if (res.code == 1) {
            top.layer.msg(res.msg);
            setTimeout(function () {
              layer.close(index);
  ...
  //待处理未签收详情弹窗
  $('#approval .item-list').on('click', '.item[data-dealWithType=claim] .item-info,.item[data-dealWithType=claim] button', function () {
    var $this = $(this).parents('.item');
    layer.open({
      title: ['审批详情', 'font-size:14px;background-color:#353b53;color:#1ae0d5'],
      type: 1,
      ...
      yes: function (index, layero) {
        ...
        var res = reqAjax(REQUEST_URL.CLAIM, JSON.stringify(param));
        if (res.code == 1) {
          layer.msg(res.msg);
          setTimeout(function () {
            layer.close(index);
          ...
          yes: function (index, layero) {
            layer.open({
              ...
              yes: function (index, layero) {
                ...
                var res = reqAjax(REQUEST_URL.COMPLETE, JSON.stringify(param))
                if (res.code == 1) {
                  layer.msg(res.msg);
                  setTimeout(function () {
                    location.reload(true);
  ...
  //已办结详情,详情按钮弹窗
  $('#approval .item-list').on('click', '.item[data-dealwithtype=havedo] .item-info,.item[data-dealWithType=havedo] button', function () {
    var $this = $(this).parents('.item');
    layer.open({
      title: ['审批详情', 'font-size:14px;background-color:#353b53;color:#1ae0d5'],
      type: 1,
