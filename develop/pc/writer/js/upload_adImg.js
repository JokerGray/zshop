function runupload($loadBtn,uploadType){
    console.log();
accessid = ''
accesskey = ''
host = ''
...
var uploader = new plupload.Uploader({
    ...
    init: {
        ...
        FilesAdded: function(up, files) {
            ...
            if(uploadType=="videoLoad"){
                $("#vdPro").html("");
                var proHtml='<div class="progress"><div class="progress-bar" style="width:0%;"><span>0%</span></div></div>';
                $("#vdPro").html(proHtml); 
            }
        },
        ...
        UploadProgress: function(up, file) {
            if(uploadType=="videoLoad"){
                ...
                $(".progress-bar span").text(file.percent+"%");                
            }            
        ...
        FileUploaded: function(up, file, info) {
            if (info.status == 200)
            {
                ...
                console.log(urls);
                if(uploadType=="imgLoad"){
                    var imgHtml='<img src="'+urls+'"/>';
                    $loadBtn.html(imgHtml);
                }else if(uploadType=="videoLoad"){
                    var vHtml='视频地址：<a id="videoUrl" href="'+urls+'" target="_blank">'+urls+'</a>'+
                                '<video id="vdhid" style="display:none;" width="450" src="'+urls+'" controls="controls"></video>';
                    $(".vedioUrl").html(vHtml);
                    $(".vedioUrl").show();                  
                }
