...
var merchantID = localStorage.getItem("merId");
if (isM == 2) {
	platForm = 1002;
}
//判断进入的是自营还是平台广告
var advType = GetQueryString('type');
if(advType == 1) {
	$("a.sidebar-item").removeClass("active");
	$("a.sidebar-item[name='self_support']").addClass('active');
	$('#expenseBox').hide();
	$('#platformCostBox').hide();
} else if(advType == 0) {
	$("a.sidebar-item").removeClass("active");
	$("a.sidebar-item[name='platform']").addClass('active');
	$('#expenseBox').show();
	$('#platformCostBox').show();
}
//数组序号
...
$(function () {
	//	获取初始化数据
	if (platForm == 1001) {
		var getData = JSON.stringify({ "platForm": platForm, "userId": 0 , "advType": advType });
	} else {
		var getData = JSON.stringify({ "platForm": platForm, "userId": userID , "advType": advType });
	}
	...
	function showData(re) {
		...
		showCity(re);
		expenseData(re);
		$('.ad_area .ad_arealiBtn[type=0]').click();
		$('.outerSour .ad_stitle_icon').click();
	}
	ExamInpNum($('#customTimeInp'));
	//给区域赋价格
	function expenseData(re) {
		var data = re.data || '';
		if (!isNull(data)) {
			var total = data.accountM || '0';
			$('#wOrange').html(splitNum(total));    //转换余额
			$('#unitPrice').html(data.labelStrategies[0].scCmsAdvStrategyPrice || '0');
			$.each(data.codeStrategy, function (i, e) {
				if (e.scCmsAdvRelationId == 0) {     //0全国     
					$('.ad_area li:nth-child(3)').attr('price', e.scCmsAdvStrategyPrice || '0');
				} else if (e.scCmsAdvRelationId == 1) {    //1省    
					$('.ad_area li:nth-child(2)').attr('price', e.scCmsAdvStrategyPrice || '0');
				} else if (e.scCmsAdvRelationId == 2) {    //2市     
					$('.ad_area li:nth-child(1)').attr('price', e.scCmsAdvStrategyPrice || '0');
				}
			})
		}
	}
	//点击付费方式和投放次数
	$('#adCost').on('click', 'li', function () {
		if($('#adCost .adCost_li_active').attr('type') == 1) {
			$('.reminder').hide();
		} else {
			$('.reminder').show();
		}
		$('li').removeClass('adCost_li_active');
		$(this).addClass('adCost_li_active');
		var tagNum = $('.showTags .tag').length > 1 ? +$('.showTags .tag').length - 1 : 0,
			tagPrice = (+$('#unitPrice').html()) * (+tagNum);
		var num = isNull($('#adTime .adTime_li_active').attr('num')) ? $('.customInpBox span').html()
		: $('#adTime .adTime_li_active').attr('num');
		calculateTotal({
			n1: $('#adPositionPrice').html(),
			n2: $('#spaceCost').html(),
			n3: $('#adSourcePrice').html(),
			n4: tagPrice,
			type: $('#adCost .adCost_li_active').attr('type'),
			num: num
		})
	})
	$('#adTime').on('click', 'li', function () {
		$('li').removeClass('adTime_li_active');
		$(this).addClass('adTime_li_active');
		$(this).hasClass('adTimeCostom') ? $('.customTimeBox').addClass('show').removeClass('hide')
			: $('.customTimeBox').addClass('hide').removeClass('show');
		var tagNum = $('.showTags .tag').length > 1 ? +$('.showTags .tag').length - 1 : 0,
			tagPrice = (+$('#unitPrice').html()) * (+tagNum);
		var num = isNull($('#adTime .adTime_li_active').attr('num')) ? $('.customInpBox span').html()
			: $('#adTime .adTime_li_active').attr('num');
	...
	//点击按钮切换价格
	$('#adType').on('click', '.mul_icon', function () {
		$('#adPositionText').html($(this).siblings('.adtype_txt').html() + '：');
		$('#adPositionPrice').html($(this).parent().attr('price'));
		var tagNum = $('.showTags .tag').length > 1 ? +$('.showTags .tag').length - 1 : 0,
			tagPrice = (+$('#unitPrice').html()) * (+tagNum);
		var num = isNull($('#adTime .adTime_li_active').attr('num')) ? $('.customInpBox span').html()
		: $('#adTime .adTime_li_active').attr('num');
	...
	$('.ad_area').on('click', '.ad_arealiBtn', function () {
		if ($(this).attr('type') == 0) {
			$('#spaceText').html('全国：');
		...
		$('#spaceCost').html($(this).parent().attr('price'));
		var tagNum = $('.showTags .tag').length > 1 ? +$('.showTags .tag').length - 1 : 0,
			tagPrice = (+$('#unitPrice').html()) * (+tagNum);
		var num = isNull($('#adTime .adTime_li_active').attr('num')) ? $('.customInpBox span').html()
		: $('#adTime .adTime_li_active').attr('num');
	...
	$('.ad_sorce').on('click', '.ad_stitle_icon', function () {
		$('#adSourceText').html($(this).siblings('.ad_stitle_txt').html() + '：');
		$('#adSourcePrice').html($(this).parent().attr('price'));
		var tagNum = $('.showTags .tag').length > 1 ? +$('.showTags .tag').length - 1 : 0,
			tagPrice = (+$('#unitPrice').html()) * (+tagNum);
		var num = isNull($('#adTime .adTime_li_active').attr('num')) ? $('.customInpBox span').html()
		: $('#adTime .adTime_li_active').attr('num');
	...
	//添加删除标签动态的计算价格
	$('.showTags').on('click', 'i', function () {
		var tagNum = $('.showTags .tag').length - 2 < 0 ? 0 : $('.showTags .tag').length - 2,
			tagPrice = (+$('#unitPrice').html()) * tagNum;
		var num = isNull($('#adTime .adTime_li_active').attr('num')) ? $('.customInpBox span').html()
			: $('#adTime .adTime_li_active').attr('num');
		calculateTotal({
	...
	$('#customTimeBtn').click(function() {
		var inputVal = $('#customTimeInp').val();
		if($('#adCost .adCost_li_active').attr('type') == 1 && (inputVal / 1000) % 1 !== 0) {
			inputVal = Math.ceil(inputVal / 1000) * 1000;
		} 
		$('.customOpeBox').html('<div class="customInpBox"><span>' + inputVal + '</span><i></i></div>');
		$('#customTimeInp').val('');
		var tagNum = $('.showTags .tag').length > 1 ? +$('.showTags .tag').length - 1 : 0,
			tagPrice = (+$('#unitPrice').html()) * (+tagNum);
		var num = isNull($('#adTime .adTime_li_active').attr('num')) ? $('.customInpBox span').html()
	...
	$('#customTimeInp').keyup(function(e) {
		if(e.keyCode == 13) {
			$('#customTimeBtn').click();
		}
	})
	$('.customOpeBox').on('click', 'i', function() {
		$(this).parent().remove();
		var tagNum = $('.showTags .tag').length > 1 ? +$('.showTags .tag').length - 1 : 0,
			tagPrice = (+$('#unitPrice').html()) * (+tagNum);
		var num = isNull($('#adTime .adTime_li_active').attr('num')) ? $('.customInpBox span').html()
	...
	function calculateTotal(obj) {
		var total = 0, num = +obj.num || 0, total = 0;
		total = (+obj.n1) + (+obj.n2) + (+obj.n3) + (+obj.n4);
		if (obj.type == 1) {
			num = Math.ceil(num / 1000);
		}
		total = num == 0 ? total : total * num;
		$('#expenseBottomNum').html((total).toFixed(2));
	}
	...
	function showAdvPlaces(re) {
		...
		for (var i = 0; i < advPlaces.length; i++) {
			var price = advPlaces[i].advStrategy || '0';
			if (price != '0') { price = advPlaces[i].advStrategy.scCmsAdvStrategyPrice || '0' }
			lihtml += '<li class="adtype_li" scCmsAdvPlaceId="' + advPlaces[i].scCmsAdvPlaceId + '" price="' + price + '">' +
		...
		if (innerType) {
			...
			for (var i = 0; i < advSources.length; i++) {
				if (advSources[i].scCmsAdvSourceType == 2001) {
					...
					console.log(typeObj+"5555");
					var type = typeObj.type, price = advSources[i].advStrategy || '0';
					if (price != '0') { price = advSources[i].advStrategy.scCmsAdvStrategyPrice || '0' }
					innerli += '<li class="inner_sorceli clear" adname="' + advSources[i].scCmsAdvSourceName + '" sccmsadvsourceid="' + advSources[i].scCmsAdvSourceId + '" type="' + type + '" price="' + price + '">' +
	...
	*/
	...
	$("#submit").on("click", function () {
		...
		subObj["ScCmsAdvAccountType"] = $('#adCost .adCost_li_active').attr('type');
		var num = '';
		num = $('#adTime .adTime_li_active').attr('num');
		if (isNull(num)) {
			num = $('.customInpBox span').html();
			if (isNull(num)) {
				layer.msg('请填写投放次数');
				return false;
			}
			if ($('#adCost .adCost_li_active').attr('type') == 1 && (+num) < 1000) {
				layer.msg('展示量投放次数最底不能少于1000次');
				return false;
			}
		}
		subObj["ScCmsAdvCount"] = num;
