(function($){
    var REQUEST_URL = {
        ...
        'query' : 'ranking/selRankingArea', //查询
        'export' : 'ranking/exportRankingArea' //导出
    };
			...
			onLoadSuccess: function (res) {
				...
				try {
					...
					for (var i = 0; i < res.data.length; i++) {
						cityName.push(res.data[i].cityName ? res.data[i].cityName : (i + 1 + ""));
						cityData.push({
							name : res.data[i].cityName ? res.data[i].cityName : (i + 1 + ""),
							value : res.data[i].merchantNum
						});
						totalNum += parseInt(res.data[i].merchantNum);
					}
					if(res.data[0].merchantTotalNum - totalNum > 0){
						cityName.push('其他');
						cityData.push({
							name : '其他',
							value : res.data[0].merchantTotalNum - totalNum
						});
					}
					//展现视图
					var option = {
