	...
	$('#table').bootstrapTable('destroy').bootstrapTable({
		...
		onLoadSuccess: function (res) {
			...
			try {
				...
				var option = echartsCustomerFunction.initOption('优惠券使用统计', ['优惠券总数量','优惠券总金额','使用总数量','使用总金额'],yData['modelType'], [yData['couponTotalNum'],yData['couponTotalAmount'],yData['couponUseTotalNum'],yData['couponUseTotalAmount']]);
			...
			if (res.code == 1) {
				if (res.data.length > 0) {
					...
					row.couponUseTotalNum = res.data[0].couponUseSum ? res.data[0].couponUseSum : '0';
					row.couponUseTotalAmount = fmoney(res.data[0].moneyUseSum ? res.data[0].moneyUseSum : '0.00');
				} 
			}
...
$("#excelbtn").click(
		function() {
			...
			try {
				var excelUrl = reqAjax('payCoupon/exportCouponUseSum', JSON
						.stringify(data));
				if (excelUrl.code != 1) {
