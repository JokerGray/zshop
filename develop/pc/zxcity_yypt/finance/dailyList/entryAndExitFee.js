...
var TableInit = function () {
    ...
    oTableInit.Init = function () {
        $('#table').bootstrapTable('destroy').bootstrapTable({
    		...
    		{field: 'accountSysName', title:'系统账户', align : "center", width: 150},
    		{ field: 'serialNo', title:'交易流水号', align : "center", width: 150 },
        { field: 'moduleName', title:'模块名称', align : "center", width: 150 },
        { field: 'payMoney', title:'支付金额',  width: 100, align : "center",
              formatter: function (res) {
                 return fmoney(res, 2);
              }
            },
    		{ field: 'taxRate', title:'税率', width: 150, align : "center",
              formatter: function (res) {
                 return res;
              }
            },
    		{ field: 'taxation', title:'税费', width: 160, align : "center" ,sortable: true,
              formatter: function (res) {
                 return fmoney(res, 2);
              }
            },
    ...
    oTableInit.queryParams = function (params) {
        var cmd = "scAlipayPoundage/selectList";
        var jsonData="";
...
//查询 进出账手续费中所以的模块
function selectModuleList(){
  var cmd = 'scAlipayPoundage/selectModuleList';
  var optionHtml = "";
  optionHtml+='<option value="-1">--选择模块--</option>';
  $.ajax({
    type : "POST",
    url : "/zxcity_restful/ws/rest",
    dataType : "json",
        ...
        if(re.code==1){
          if(re.data[0]==null){
            ...
            for (var i = 0; i < re.data.length; i++) {
              optionHtml+='<option value="'+re.data[i].moduleId+'">'+re.data[i].moduleName+'</option>';
            }
            $("#selectview2").empty().append(optionHtml);
            $('#selectview2').selectpicker('render');
            $('#selectview2').selectpicker('refresh');
    ...
    error : function(re) {
      optionHtml+='<option value="-1">系统繁忙</option>';
      $("#selectview2").empty().append(optionHtml);
      $('#selectview2').selectpicker('render');
      $('#selectview2').selectpicker('refresh');
    }
...
function getSearchParam(pageNo, pageSize){
  ...
  $("#excelbtn").bind("click", function downloadFile(){
      ...
      var excelUrl = reqAjax('scAlipayPoundage/exportScAlipayPoundage', jsonData);
      if(excelUrl.code != 1){
