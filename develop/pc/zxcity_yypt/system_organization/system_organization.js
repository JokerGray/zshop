(function($){
                ...
                for (var i = 0; i < rowA.children.length; i++) {
                    ...
                    for (var j = 0; j < rowB.children.length; j++) {
                        ...
                        for (var k = 0; k < rowC.children.length; k++) {
                            ...
                            sHtml += '<li data-id="' + rowD.id + '"><span class="glyphicon glyphicon-circles"></span>' +rowD.name + '</li>';
                        };
                        sHtml += '</ul>' +
                        '</li>';
                    };
    ...
    function systemDetail(res) {
        ...
        if (res.code == 1) {
            for (var i = 0; i < res.data.length; i++) {
                ...
                '<td class="row remove-modifier" width="13%">' +
                '<div class="cagbtn">' +
    ...
    function getSystem() {
        ...
        if(pid == undefined || pid == '' || pid=== 'null' || pid=== null){
            var paramDetail = "{'page':" + page + ",'rows':" + rows + ",'name':'" + val + "','sort':'sort','order':'asc'}"; //查询权限列表
        }else{
            var paramDetail = "{'page':" + page + ",'rows':" + rows + ",'name':'" + val + "','pid':'" + pid +  "','sort':'sort','order':'asc'}";  //查询权限列表
        }
    ...
    //模拟渲染
        ...
        layer({
            ...
            ,jump: function(obj,first){
                if(pid == undefined || pid == '' || pid=== 'null' || pid=== null){
                    var paramDetail = "{'page':" + obj.curr + ",'rows':" + rows + ",'name':'" + val + "','sort':'sort','order':'asc'}"; //查询权限列表
                }else{
                    var paramDetail = "{'page':" + obj.curr + ",'rows':" + rows + ",'name':'" + val + "','pid':'" + pid +  "','sort':'sort','order':'asc'}";  //查询权限列表
                }
                    var res=reqAjax(USER_URL.SYSTEMLIST,paramDetail);
                    systemDetail(res);
    ...
    //保存方法
    function sub(index,type,pid) {//type 为3则添加保存，2为修改保存
        ...
        var systemSort = $.trim(formData.find('form').find('#systemLevel').val() != "" ? formData.find('form').find('#systemLevel').val():""); //优先级
        if (type == 2) {
    ...
    //修改
   $("#system-table").on("click", ".cagbtn", function () {
