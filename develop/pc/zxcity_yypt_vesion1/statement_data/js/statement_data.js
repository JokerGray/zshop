(function($){
    ...
    var USER_URL =  {
        RESOURLIST : 'operations/statisticsAll', //(总量统计)
        EACHEATS : 'operations/statisticsTimeLineAll' //(N天之内的接听量)
    };


    $(function(){
        intLine('myEchars',"1",yesteaday,yesteaday);
   ...
   function intLine(e,type,beginTime,endTime){
       ...
       if(type==0){
        ...
        var data = res.data;
            for(var k=data.length-1;0<=k&&k<=data.length;k--){
                seriesData.push(data[k].allNum);
                xAxisData.push(data[k].indate)
    ...
    function getData(begintime,endtime){
        ...
        reqAjaxAsync(USER_URL.RESOURLIST,JSON.stringify(param)).then(function(res){
            if(res.code==1){
                var row = res.data;
                $(".down-content .down-list").eq(0).find(".list-num").text(row.callLogNub);
                $(".down-content .down-list").eq(1).find(".list-num").text(row.newsNum);
                $(".down-content .down-list").eq(2).find(".list-num").text(row.avgTalkTime);
                $(".down-content .down-list").eq(3).find(".list-num").text(row.avgSatisfaction);
                $(".down-content .down-list").eq(4).find(".list-num").text(row.lineupNum);
                $(".down-content .down-list").eq(5).find(".list-num").text(row.notAnswering);
            }else{
                layer.msg(res.msg);
            }
        })
