...
function filter(data) {
  ...
  } else if (data.flag == 'img' || data.flag == 'cover') {
    obj = [{
      title: "Image files",
      extensions: "jpg,png,jpeg"
    }]
  } else if (data.flag == 'gif') {
    obj = [{
      title: "Image files",
      extensions: "gif"
    }]
      ...
      FileUploaded: function (up, file, info) {
        if (info.status == 200) {
          if (data.flag == "video") {
            ...
            return sendImageCallback(file, data);
          } else if (data.flag == 'gif') {
            return send720Callback(file, data);
          }
        }
      },
      Error: function (up, err) {
        if (err.code == -600) {
          layer.msg("选择的文件过大,请重新选择!");
...
function sendAddImageCallback(file, data) {
  ...
  $('#uploadImg').html('<img src="' + urls + '">');
  parent.layer.close(_uploadLoading);
}

//图片
function sendImageCallback(file, data) {
  $('#cut').show();
  layer.msg("上传成功");
  var urls = uploadJson.host + "/" + get_uploaded_object_name(file.name);
  var imgs =
    '<img class="editor_img" src="' + urls + '" width="550">' +
    '<img class="editor_imghid" src="' + urls + '" style="display:none;">' +
    '<img class="showArea" style="display: none;float: right;" width="550">'
  $('.img-show').html(imgs);
  parent.layer.close(_uploadLoading);
}

//720封面
function send720Callback(file, data) {
  $('#cut').show();
  layer.msg("上传成功");
  var urls = uploadJson.host + "/" + get_uploaded_object_name(file.name);
  var imgs =
