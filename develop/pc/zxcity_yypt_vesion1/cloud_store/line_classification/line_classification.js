...
//列表初始化方法
  function initList() {
    ...
    var res = reqAjax(REQUEST_URL.GETONLINECATEGORYLIST, JSON.stringify(param));
    getList(res);
...
//设置线上分类是否首页显示
  $('#line_classification').on('click', '.isShow', function () {
    ...
    var res = reqAjax(REQUEST_URL.SETCATEGORYINHOME, JSON.stringify(param));
    if (res.code == 1) {
      layer.msg(res.msg);
      setTimeout(function () {
        location.reload();
...
//删除方法
  $('#line_classification').on('click', '.delete', function () {
    ...
    layer.confirm("确定删除吗?", {icon: 3, title: '提示'}, function () {
      var res = reqAjax(REQUEST_URL.REMOVEONLINECATEGORY, JSON.stringify(param));
      if (res.code == 1) {
        layer.msg(res.msg);
        setTimeout(function () {
          location.reload();
  ...
  $('#line_classification').on('click', '.edit', function () {
    ...
    layer.open({
      ...
      success: function (layero, index) {
        ...
        body.find('#uploadImg').html('<img src="' + categoryLogo + '">');
...
//保存方法
  function sub(id) {
    ...
    var res = reqAjax(REQUEST_URL.SUBONLINECATEGORY, JSON.stringify(param));
    if (res.code == 1) {
      layer.msg('保存成功');
      setTimeout(function () {
        location.reload();
