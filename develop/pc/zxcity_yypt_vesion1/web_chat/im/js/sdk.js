...
var SDKBridge = function (ctr,data) {
	...
	function onDisconnect(error) {
	    ...
	    if (error) {
	        ...
	        case 302:
				top.window.layer.msg(error.message);
		    	delCookie('uid');
			    delCookie('sdktoken');
				top.window.misc.quit()
			    window.location.href = 'login.html';
	        ...
	        case 'kicked':
				...
				var str =error.from;
				top.window.layer.msg("你的账号于"+dateFormat(+new Date(),"HH:mm")+"被"+(map[str]||"其他端")+"踢出下线，请确定账号信息安全!");
			    delCookie('uid');
			    delCookie('sdktoken');
				top.window.misc.quit()
			    window.location.href = login.html;
...
SDKBridge.prototype.kick = function(type){
	...
	this.nim.kick({
	    deviceIds: [deviceIds],
			...
			layer.msg('踢'+(type===0?'移动':'PC')+'端' + (!error?'成功':'失败'));
		    console.log(error);
		    console.log(obj);
	    }
	});
