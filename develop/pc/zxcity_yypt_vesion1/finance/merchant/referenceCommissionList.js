...
var levelTypeList
		...
		$('#table').bootstrapTable({
			...
			columns : [
				...
				{field: 'referenceAuditUserName',title:'操作',width: 10,align : "center",
	                formatter: function(res,data){
	                	return '<button onclick="edit('+data.id+')" type="button" class="btn btn-info btn-sm edit" aria-hidden="true" style="padding-top: 0; padding-bottom: 0;"">查看</button>';
	                }
	            }
			],
			onLoadSuccess: function (data) {
				...
				$.each(data.data.LevelTypeList,function(i,item){
	...
	table.param = function(params) {
		...
		var data = {
			...
			referenceAuditTimeStart: startTime,
			referenceAuditTimeEnd: endTime,
	...
	edit = function(id) {
		...
		var shareRealAmount = data.shareRealAmount;
		var shareTaxation = data.shareTaxation;
		...
		var shareRealAmount = (shareAmount * 100 - shareTaxation * 100)/100;
		shareRealAmount = Math.round(parseFloat(shareRealAmount) * 100) / 100;
