				...
				},
				{
					...
					formatter : function(res,data) {
						if (res == "0") {
							...
							return '<span style="color: gray">非员工</span>';
						}
					}
				},
				{
	...
	upgradeDetail = function(id) {
		...
		var aliPayPoundage = (upgradeAmount * 100 * 0.055)/100;
		...
		// 加载模板
		html = template('onLineStaffListTpl',data);
		// 打开弹窗	
		layer.open({
			type : 1,
			title : '商户升级详情',
	...
	commission = function(id) {
		...
		// 加载模板
		html = template('OnlineEmployeeCommissionTpl',data);
		// 打开弹窗	
		layer.open({
			type : 1,
			title : '线上员工佣金发放',
