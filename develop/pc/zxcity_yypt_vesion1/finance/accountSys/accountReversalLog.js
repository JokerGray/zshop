...
var TableInit = function () {
  ...
  oTableInit.Init = function () {
    $('#table').bootstrapTable('destroy').bootstrapTable({
      ...
      columns: [
        ...
        {field: 'reversalTypeStr', title: '倒账类型', sortable: true, width: 120, align: "center"},
  ...
  oTableInit.queryParams = function (params) {
    var cmd = "accountReversal/selectList";
...
$("#addbtn").bind("click", function () {
  var index =layer.open({
        ...
        btn1 : function(index) {
        	var reversalType = $("#select1").val();
            var money = $("#money_").val();
            var remark = $("#remark_").val();
            
            if(!reversalType || reversalType == -1){
            	layer.msg("请选择倒账类型！");
            	return false;
            }else if(!money){
            	layer.msg("请输入金额！");
	        ...
	        var cmd = 'accountReversal/addReversalLog';
...
function  deletebtn(id){
  ...
  function(index) {
    ...
    var indexs = layer.load(0,{shade: [0.7, '#393D49']}, {shadeClose: true}); //0代表加载的风格，支持0-2
    var cmd = 'accountReversal/deleteLog';
...
$("#excelbtn").bind("click", function downloadFile() {
  ...
  try {
    ...
    var excelUrl = reqAjax('accountReversal/exportReversalLogs', jsonData);
    //2.获取下载URL
...
function addInfo(infoName){  
	  ...
	  html+='<label class="col-md-4 col-sm-4 control-label"><b class="required-icon">*</b>倒账类型：</label>';
	  ...
	  html+='<option value="1">支付宝到微信</option>';
	  html+='<option value="2">微信到支付宝</option>';
	  html+='</select>';
	  html+='</div>';
	  html+='</div>';
	  html+='<div class="form-group">';
