...
var TableInit = function () {
    ...
    oTableInit.Init = function () {
        $('#table').bootstrapTable('destroy').bootstrapTable({
            ...
            columns: [
                ...
                { field: 'extractTypeStr', title:'收入类型', width: 150, align : "center",
                    formatter: function(res){
                        if(res==null){
                            return res="-";
                        }else{
                ...
                { field: 'extractMoney', title:'提取金额', width: 150, align : "center",
					formatter: function(money){
						return fmoney(money, 2);
					}
				},// 金额格式化  // 提取金额
				{ field: 'extractTime', title:'提取时间',  sortable: true, width: 160, align : "center" },// 提取时间
				...
				{ field: 'yyUserName', title:'提取人', width: 150, align : "center", 
					formatter: function(res){
                        if(res==null){
                            return '-';
                        }else{
    ...
    oTableInit.queryParams = function (params) {
        var cmd = "profitExtractLog/selProfitExtractLog";
        var pageNo = params.offset / params.limit + 1;
        var pageSize = params.limit;
        var version = "1";
        var serialNo = $('#serial').val();
        var extractType = $("#selectview2").val();
        //var selectplate=$("#selectplate option:selected").text();
        var startTimes=$('#qBeginTime').val();
        var endTimes=$('#qEndTime').val();

        //jsonData = "{'extractType':'"+extractType+"','extractTimeStart':'"+startTimes+"','extractTimeEnd':'"+endTimes+"','pageNo':"+pageNo+",'pageSize':"+pageSize+"}";

        var jsonData = {
        		extractType: extractType,
        		extractTimeStart: startTimes,
        		extractTimeEnd: endTimes,
        		serialNo: serialNo,
        		pageNo: pageNo,
        		pageSize: pageSize
            }
    ...
    $("#excelbtn").bind("click", function downloadFile() { 
        ...
        try{
            ...
            jsonData = "{'extractType':'"+extractType+"','extractTimeStart':'"+startTimes+"','extractTimeEnd':'"+endTimes+"'}";

            var excelUrl = reqAjax('payExcel/exportProfitExtractLog', jsonData);
            if(excelUrl.code != 1){
