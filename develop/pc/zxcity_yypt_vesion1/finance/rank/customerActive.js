(function($){
	...
	function chenkInput() {
		...
		var recharge = $("#recharge").val() ? $("#recharge").val() : null;
		if(totalMoney && !is_amount(totalMoney) ){
			$("#expense").val('');
			layer.msg("请输入正确次数!");
			return true;
		}
		if(recharge && !is_amount(recharge) ){
			$("#recharge").val('')
			layer.msg("请输入正确次数!");
			return; true;
		}
		...
		return {
			...
			data : JSON.stringify({
				dataTable : {
					...
					consumptionTime : parseInt($("#expense").val()),
					rechargeTime : parseInt($("#recharge").val())
				},
				pagination : {
