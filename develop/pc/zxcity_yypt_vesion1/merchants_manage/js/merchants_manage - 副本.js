(function($) {
	...
	function getData(url, parms) {
		...
		$.each(data, function(i, item) {
			...
			if(item.merchantStatus == 1) { //是否锁定  1 否 0 是
				$(item).attr('merchantStatusName', '否');
			} else if(item.merchantStatus == 0) {
				$(item).attr('merchantStatusName', '是');
			};
			if(item.orgLevel == 1) {
		...
		//查看
			...
			layer.open({
				...
				shade: [0.1, '#fff'],
					...
					$('#yyzzback').attr('src',"img/upload.png");
					$('#yyzzfont').attr('src',"img/upload.png");
					$('#lookDemo').hide();
				},
				...
				reqAjaxAsync(USER_URL.GETCHARGELEVELBY,JSON.stringify(timeList)).then(function(res){
					...
					if(res.data.orgLevel == '1'){
						$('#orgLevelSee').val('普通商户')
					}else if(res.data.orgLevel == '2'){
						$('#orgLevelSee').val('合作商户')
					}else if(res.data.orgLevel == '3'){
						$('#orgLevelSee').val('代理商户')
					}
				})
				
				$('#yyzzfont').prop('src',data.licenseFront);
				$('#yyzzback').prop('src',data.licenseHold);
					...
					yes: function(index, layero) {
						form.on('submit(formdemoOne)', function(data) {
							if(data) {
								...
								var moneyMode = $('input[name="moneyMode"]:checked').val();
								var shareUserId = 0;
								var shareUserLevel = 1;
