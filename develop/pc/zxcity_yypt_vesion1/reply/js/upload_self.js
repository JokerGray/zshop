    ...
    var uploaderLabel = new plupload.Uploader({
        ...
        init: {
                ...
                if (data.flag == "sendLocal") {
                    ...
                    reader.onload = (function (e) {
                        ...
                        image.onload = function () {
                            var oldwith = $("#cutbtn").find(".leftwidth").text();
                            var oldheight = $("#cutbtn").find(".leftheight").text();
                            if (this.width < oldwith || this.height < oldheight) {
                                layer.msg('请上传宽不小于' + oldwith + ';高不小于' + oldheight + '的图片');
                            } else {
            ...
            UploadProgress: function(up, file) {
                if (data.flag == "sendLocal" || data.flag == "video" || data.flag == "editorVideo" || data.flag == "cover" || data.flag =="excel") {
            ...
            FileUploaded: function(up, file, info) {
                if (info.status == 200) {
                    if (data.flag == "reg") {
                        ...
                        return sendExcelCallback(file, data);
                    }  else {
                        console.log("请传递标记");
                    }
                } else {
...
function sendLocalCallback(file, data) {
    ...
    var urls = uploadJson.host + "/" + get_uploaded_object_name(file.name);
    localStorage.setItem("urls", urls);
    var width = $("#cutbtn .leftwidth").text();
    ...
    var img = "<div class='item_box fl'>" +
        "<img src='" + urls + "' class='editor_img' width='600' />" +
        "<img style='display:none' src='" + urls + "' class='editor_imghid' />"  +
        "<i class='deleteI'></i></div>";
...
function sendExcelCallback(file, data) {
    layer.msg("上传成功");
    var urls = uploadJson.host + "/" + get_uploaded_object_name(file.name);
    var img = "<div class='item_box fl'><a href='"+ urls +"'><i class='fa fa-file-excel-o'></i><span>"+ file.name +"</span></a><i class='deleteI'></i></div>";
    $("#" + data.imgDom).html(img);
}