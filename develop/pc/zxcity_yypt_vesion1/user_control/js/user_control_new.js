(function($) {
  ...
  var locked = true;
  var administration = sessionStorage.getItem('administration') || ""; //1-管理员 0-非管理员
//接口参数
  var USER_URL = {
    ...
    UPDATEPAS : 'operations/updatePwd', //修改密码
...
//重置
  ...
  function tableInit(tableId, cols, pageCallback, test,orgid) {
    ...
    //2.第一次加载
    var res = pageCallback(1, 15,orgid);
    //第一页，一页显示15条数据
    if(res) {
      if(res.code == 1) {
        tableIns.reload({
  ...
  function getData(url,parms){
    ...
    $.each(data,function(i,item){
      $(item).attr('eq',(i+1));
      $(item).attr('administration',administration);
  ...
  function getTable(username,phone,organizationId){
    ...
    var initTable = obj.tableIns;
      var res = pageCallback(1, 15,username,phone,organizationId);
...
//单个删除
  ...
  $('#updateBtn').click(function () {
      console.log('1111')
      layer.confirm('是否更新员工绑定状态?',{icon: 0, title: "提示"},function (index) {
          reqAjaxAsync(USER_URL.UPDATEPERSON).done(function (res) {
              if(res.code == 1){
                  layer.confirm('更新'+res.data+'条绑定状态',{btn:['确定']},function (index) {
   ...
   layer.open({
     ...
     offset: '20px',
      ...
      success:function(layero,index){
        $("#nicName").css("display","block");
        $("#nicName input").attr("lay-verify","required");
        ...
        form.on('select(role)', function(data){
          console.log($(this).text()); //得到select原始DOM对象
         if($(this).text()=="客服人员"){
           $(".workphone").find("b").show();
           $("#phoneWork").attr("lay-verify","required|phone");
         }else{
           $(".workphone").find("b").hide();
           $("#phoneWork").attr("lay-verify","");
         }
        });
        ...
        form.on('submit(formDemo)', function(done){
            if(done){
             ...
             var orgs= $('.loginName').attr("data-id");
              var nickname = $.trim($(".nicName").val());//昵称
             ...
             // debugger
              ...
              if(!orgs || orgs == ""){
                ...
                var workphone = $.trim($("#phoneWork").val()); //工作号码
                var param ={
                  name:loginName,
                  uname:surname,
                  sex:sexchoose,
  ...
  table.on('tool(table)', function(obj){
    ...
    if(obj.event === 'change'){
      ...
      layer.open({
        ...
        success: function(layero, index){
          $("#nicName").css("display","none");
          $("#nicName input").attr("lay-verify","");
        /* $(".upwd").css("display","none");*/
          ...
          $("#surname").val( uname);
          //新增了工作号码
          $("#phoneWork").val(data.workphone);
          if(data.role_name=="客服人员"){
            $(".workphone").find("b").show();
            $("#phoneWork").attr("lay-verify","required|phone");
          }else{
            $(".workphone").find("b").hide();
          ...
          form.on('submit(formDemo)', function(done) {
            if (done) {
             ...
             /* var flgs = $("#surname").attr("data-flag");*/ //判断是否为父节点
              ...
              var workphone = $.trim($("#phoneWork").val());
    ...
    //查看
      ...
      layer.open({
        title: ['绑定代理商用户', 'font-size:12px;background-color:#0678CE;color:#fff'],
        type: 1,
      ...
      //绑定app
      layer.open({
        title: ['绑定App用户', 'font-size:12px;background-color:#0678CE;color:#fff'],
        type: 1,
        content:$('#blindApp'), //这里content是一个DOM，注意：最好该元素要存放在body最外层，否则可能被其它的相对元素所影响
        area: ['800px', '650px'],
        btn:['绑定','取消'],
    ...
    }else if(obj.event === 'canbtn'){ //修改密码
      var id = data.id;
      layer.open({
        title: ['修改密码', 'font-size:12px;background-color:#0678CE;color:#fff'],
        type: 1,
        ...
        yes:function(index, layero) {
          ...
          form.on('submit(formdemo)',function(data){
            if(data) {
              ...
              if (locked) {
                locked = false;
                reqAjaxAsync(USER_URL.UPDATEPAS, JSON.stringify(param)).done(function (resq) {
                  if (resq.code == 1) {
                    layer.msg(resq.msg);
                    layer.close(index);
                    var organi = sessionStorage.getItem('organizationId');
