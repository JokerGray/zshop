...
;(function(root, factory) {
    ...
    jedfn.init = function(){
        ...
        var formatDate = function (cls,boxcell) {
            ...
            //2017-07-12
            //为点击input框切换日历显示与隐藏添加参数设置
            $(that.valCell).attr("data-show", true);
        };
        //为开启初始化的时间设置值
        ...
        if($(fixedCell).length > 0){
            ...
            if (jetrigger != false) {
                that.valCell.on(jetrigger, function (ev) {
                    ...
                    //2017-07-12
                    //为点击input框切换日历显示与隐藏添加参数设置
                    if(!$(that.valCell).attr("data-show") || "false" == $(that.valCell).attr("data-show")){
                        formatDate(jd[0],jd[1]);
                    }else{
                        $(that.valCell).attr("data-show", false);
                        return;
                    }
                });
            }else {
    ...
    jedfn.dateClose = function() {
        if($($(jet.boxelem).attr(jefix)).length == 0) {
            ...
            //2017-07-12
            //为点击input框切换日历显示与隐藏添加参数设置
            $(this.valCell).attr("data-show", false);
        }
    };
    // 提示层
    ...
    jedfn.eventsDate = function(opts,boxCell) {
        ...
        $(document).on("mouseup scroll", function(ev) {
            ...
            if (jet.boxelem == "#jedatebox"){
                ...
                //2017-07-12
                //为点击input框切换日历显示与隐藏添加参数设置--比对元素id
                if($(that.valCell).attr("id") != ev.target.id){
                    $(that.valCell).attr("data-show", false);
                }
