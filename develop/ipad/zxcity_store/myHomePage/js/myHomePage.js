...
var newAccount = function(){
    ...
    if(res.code == 1){
        ...
        $("#monthRegister").html(res.data.monthRegister - res.data.todayRegister);
...
var cardWarning = function(){
    ...
    if(res.code == 1){
        ...
        $(".module-6 .module-con .row").html(sHtml);

        $(".module-6 .module-con .row").delegate(".item .sub-title a", 'click', function(){
...
var myHomePageLargess = function(){
    ...
    if(res.code == 1){
        ...
        $("#monthLargess").html(res.data.monthLargess-res.data.todayLargess);

        var sendCountParams = {
            'legendArr':['今日赠送','本月已赠送'],
            'title':'赠送统计',
            'data':[
                {value:res.data.todayLargess, name:'今日赠送'},
                {value:(res.data.monthLargess-res.data.todayLargess), name:'本月已赠送'}
            ]
        };
...
$(".module-8 .item .count-num").click(function(){
    var tit = $(this).siblings('.sub-title').text();
...
$(".module-3 .module-title a, .module-4 .module-title a").click(function(){
    window.parent.location.href = "../../statisticalReport/html/consultantPerformanceDetail.html?salesmanId="+employeeId+"&merchantId="+merchantId+"&salesman="+employeeName
});
