(function($){
	...
	var REQUEST_URL = {
		'bar_url':'shopStatistics/queryShopAchInfoSummeryDetials',
		'pie_url':'shopStatistics/queryShopProInfoSummeryDetials'
	};
	...
	$("#salsumptimeTab").find("li").click(function() {
		...
		if (thisIndex == 3){
			...
			$("#salsumpHistogramDate").find(".start-time").val(today);
			$("#salsumpHistogramDate").find(".end-time").val(today);
		}
	});
	//柱状图日期筛选验证事件
	...
	$("#salsumptimePieTab").find("li").click(function() {
		...
		if(index == 3){
			...
			$("#salsumpPieDate").find(".start-time").val(today);
			$("#salsumpPieDate").find(".end-time").val(today);
		}
	});
