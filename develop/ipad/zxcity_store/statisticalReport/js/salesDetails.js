...
function getDate(start, end, sName, pageNo, pageSize) {
	var cmd = "shop/querySalesSummeryDetials";
...
//点击客户姓名跳转详情页  项目编号：93 项目人：WK
function handleCustomerName(){
	var dataLi=$('#sealsdetails').find('tr');
	...
	if (resData.code == 1) {
		if (resData.total > 0) {
			...
			for (var i = 0; i < obj.length; i++) {
				sHtml += '<tr>';
				sHtml += '<td>' + obj[i].salesman + '</td>';
				sHtml += '<td>' + obj[i].serviceName + '</td>';
				sHtml += '<td>' + obj[i].price + '</td>';
				sHtml += '<td data-acId='+obj[i].accountId+'>' + obj[i].accountName + '</td>';
