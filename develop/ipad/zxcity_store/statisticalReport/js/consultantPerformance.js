...
$(function() {
  ...
  //设置时间默认时间为今天
	...
	$("#searchBtn").click(function() {
		...
		var end = $("#endTime").val();
		var advName = $("#adviserName").val();
		var resData = getDate(start, end, advName, 1, 10);
		fillList(resData);
		$('#page-selection').bootpag({
			total : Math.ceil(resData.total / 10),
			page : 1,
			...
			resData = getDate(start, end, advName, num, 10);
			fillList(resData);
		});
	});
});

function getDate(start, end, advName, pageNo, pageSize) {
	var cmd = "shop/queryAdviserSummeryDetials";
	var data = "{'merchantId': " + merchantId + ",'startData':'" + start
			+ "','endData':'" + end + "','adviserName':'" + advName
	...
	if (resData.code == 1) {
		if (resData.total > 0) {
			...
			for (var i = 0; i < obj.length; i++) {
				sHtml += '<tr onclick="showDetail(' + obj[i].salesmanId +',\''+obj[i].salesman+ '\')">';
				sHtml += '<td>' + getStringFomat(obj[i].salesman) + '</td>';
...
function showDetail(salesmanId,salesman) {
	var start = $("#startTime").val();
	var end = $("#endTime").val();
