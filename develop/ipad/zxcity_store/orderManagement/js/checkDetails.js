(function($){
	var tabVal = getUrlParams('tabVal');
	var orderId = getUrlParams('orderId');

	if(tabVal != null && orderId != null){
		loadDetails(tabVal,orderId);
	}

	function loadDetails(tabVal,orderId){
		var jsonData = "{'orderId':"+orderId+"}";
		var res;
		if(tabVal == "0"){
			res = reqAjax("shop/presaleDetails", jsonData);
		}else{
			res = reqAjax("shop/orderDetails", jsonData);
		}
		var sHtml = "";
		if(res.code == 1){
			var obj = res.data;
			if(obj.length > 0){
				for(var i=0; i<obj.length; i++){
					sHtml += '<li class="list-item">'
						...
						+ '<h4 class="media-heading" title="'+obj[i].purchaseName+'">'+obj[i].purchaseName+'</h4>'
