    ...
    function getPositionListByMerchantId(){
		...
		var resData = reqAjax('shopOrder/findPositionListByMerchantId', jsonData);
        if(resData.code == 1){
            var obj = resData.data, sHtml = '';
            for(var i=0; i<obj.length; i++){
                if(obj[i].positionName.search('厨师') > -1){
                    sHtml += '<option value="'+obj[i].id+'" selected>'+obj[i].positionName+'</option>';
                }else{
        ...
        setTimeout(function(){
            ...
            $("#todoList .todo-dishes-list").droppable({
                ...
                drop:function(event, ui){
                    ...
                    var resData = reqAjax(cmd, param);
                    if(resData.code == 1){
                        layer.msg("操作成功！");
                        var sHtml = '<li id="'+orderId+'" class="todo-dishes">'
                            + '<div class="dishes-info">'
                            + '<p class="name">菜品名称：<span>'+orderName+'</span></p>'
                            + '<p>菜品备注：<span>'+remark+'</span></p>'
                            + '<p>下单时间：<span>'+orderTime+'</span></p></div>'
                            + '<div class="handle-box">'
                            + '<a class="take-btn" href="javascript:void(0);">取菜</a>'
    ...
    function getPersonListByPositionId(){
		...
		var resData = reqAjax('shopOrder/findBackUserListById', jsonData);
        if(resData.code == 1){
            var obj = resData.data, sHtml = '';
            for(var i=0; i<obj.length; i++){
                ...
                if(todoList.length > 0){
                    for(var j=0; j<todoList.length; j++){
                        ...
                        sHtml += '<li id="'+todoList[j].id+'" class="todo-dishes">'
                            + '<div class="dishes-info">'
                            + '<p class="name">菜品名称：<span>'+todoList[j].purchaseName+'</span></p>'
                            + '<p>菜品备注：<span>'+remark+'</span></p>'
    ...
    function loadOrderList(){
        var jsonData = "{'merchantId':"+merchantId+",'shopId':"+shopId+",'positionId':"+positionId+"}";
		var resData = reqAjax('shopOrder/findPresaleInfoListById', jsonData);
        if(resData.code == 1){
            var obj = resData.data, sHtml = '';
            for(var i=0; i<obj.length; i++){
