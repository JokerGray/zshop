$(function() {
						...
						if (!$(this).hasClass("active")) {
							...
							$(".J_mainContent .J_iframe").each(function() {
								if ($(this).data("id") == o) {
									...
									//刷新iframe页面的table
									try {
										$(this)[0].contentWindow.refreshTable();
									} catch (e) {
									}
	...
	function h() {
		...
		if ($(this).parents(".J_menuTab").hasClass("active")) {//这个tab是活跃的
			if ($(this).parents(".J_menuTab").next(".J_menuTab").size()) {//后面还有tab
				//取后面的第一个tab
				var k = $(this).parents(".J_menuTab").next(".J_menuTab:eq(0)").data("id");
				//后面的第一个tab变为活跃
				$(this).parents(".J_menuTab").next(".J_menuTab:eq(0)").addClass("active");
				//选中后面第一个tab对应的iframe进行显示，其他的隐藏
				$(".J_mainContent .J_iframe").each(function() {
					if ($(this).data("id") == k) {
						$(this).show().siblings(".J_iframe").hide();
						/**
			...
			/**
			 * 这个tab的前面还有tab,才进行下面的操作,
			 * 对于前面一个if进行了操作，那后面这个不加else也不会进入,因为size一定为0
			 */
	...
	$(".J_tabCloseAll").on("click",function() {
		$(".page-tabs-content").children("[data-id]")
//		.not(":first")
		.each(function() {
			$('.J_iframe[data-id="'+ $(this).data("id")+ '"]').remove();
			$(this).remove();
